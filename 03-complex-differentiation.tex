...
\[
a<b_k\ (\forall k)\Longrightarrow a<\inf_k b_k,
\qquad
a_k<b\ (\forall k)\Longrightarrow\sup_k a_k<b.
\]

\subsubsection*{Application — Complex differentiation}
Before seeing some good instances of Tool 5, let us have a brief exposition on complex differentiation.
...
As metric spaces, $\mathbb C$ and $\mathbb R^2$ are homeomorphic; as vector spaces, they are isomorphic. But do these two definitions match? They ought to because $\mathbb C\cong\mathbb R^2$, and functions on $\mathbb R^2$ can be seen as functions on $\mathbb C$ and vice versa. As it turns out, a differentiable function on $\mathbb R^2$ need not be complex differentiable. Why do they not match, and how do


we reconcile them? Let \(f\colon\mathbb R^2\longrightarrow\mathbb R^2\)
...
\]
These are precisely the Cauchy--Riemann equations. We demand that an $\mathbb R^2$-differentiable function satisfy the Cauchy--Riemann equations. Now that we know precisely what we demand, let us walk our way from


$\mathbb R^2$-differentiability, or Fr\'echet differentiability, to complex differentiability.
...
\textbf{Remark.}
As we saw with the construction of $\mathbb C$, the algebraic structure imposed rigidity on it. We shall see that exploiting the algebraic structure of $\mathbb C$ to define a notion of differentiability makes this class of functions, often called holomorphic functions, far more regular than Fr\'echet differentiable functions, or even real differentiable functions.
...
\textbf{Remark.}
A quick realization of the fact that holomorphic functions are regular, far more than Fr\'echet differentiable functions, can be seen in their derivative. If $f=u+iv$, then
\[
f'(z_0)=
\begin{pmatrix}
u_x&u_y\\
v_x&v_y
\end{pmatrix}
\binom{\operatorname{Re}(z_0)}{\operatorname{Im}(z_0)}.
\]
Note that the matrix is orthogonal, since \(u_xu_y+v_xv_y=0\)
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=.46\linewidth]{note1-fig-04.pdf}
  \textit{Figure N1.4}
\end{figure}
...
\right\rangle.
\end{align*}
As $f'$ is orthogonal, it fixes inner products, so
\[
\langle(f\circ\gamma_1)'(t_0),(f\circ\gamma_2)'(t_0)\rangle
=\langle\gamma_1'(t_0),\gamma_2'(t_0)\rangle.
\]
...
Thus $f\in H(\Omega)$ does not deform curves haphazardly: it does so in such a manner that neither the rate of variance nor the angle of intersection changes. If $f\in H(\Omega)$ and $f'\neq0$ on $\Omega$, then $f$ is called a conformal map. Mind you, these maps are very rare. In this theme there is one result of relevance:

The only smooth conformal maps on $\mathbb R^n\cup\{\infty\}$ are the M\"obius transformations.
% BLOG-CONTENT-END
\end{document}
