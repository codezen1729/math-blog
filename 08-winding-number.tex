...
% BLOG-CONTENT-BEGIN
% BLOG-PASSAGE: winding-number:p0001
Let us look at singularities through contour integration.

% BLOG-PASSAGE: winding-number:p0002
\subsection*{Recall: Cauchy's Theorem}
If \(\gamma_0:[0,1]\to U\) and \(\gamma_1:[0,1]\to U\) are two closed rectifiable curves homotopic in $U$ and \(f\in H(U)\), then
\[
  \int_{\gamma_0} f(z)\,dz=\int_{\gamma_1} f(z)\,dz.
\]

% BLOG-PASSAGE: winding-number:p0003
...
% BLOG-PASSAGE: winding-number:p0004
...
% BLOG-PASSAGE: winding-number:p0005
...
% BLOG-PASSAGE: winding-number:p0006
Integrating a holomorphic function over curves in different homotopy classes can yield different values. To make every closed-curve integral vanish, assume that \(U\) is simply connected. Note here that if \(\gamma:[0,1]\to U\) is any closed curve, then
\[
  \int_\gamma f(z)\,dz=0.
\]
...
\[
  \frac{H(z)}{z-z_0}.
\]
We know locally that \(H(z)=H(z_0)+(z-z_0)H'(z_0)+\cdots,\)
and hence
\[
  \frac{H(z)-H(z_0)}{z-z_0}=H'(z_0)+\cdots=:g(z).
\]
...
% BLOG-PASSAGE: winding-number:p0007

Let us first focus on
\[
  g(z)=\frac{H(z)-H(z_0)}{z-z_0}.
\]
Clearly, \(g\in H(U\setminus\{z_0\})\) and is bounded near \(z_0\), which can be seen from the linear approximation of \(H\) near \(z_0\).

% BLOG-PASSAGE: winding-number:p0008
...
Now, if we estimate,
\[
  \left|\int_{\gamma'} f(z)\,dz\right|<M\cdot2\pi\varepsilon
...
% BLOG-PASSAGE: winding-number:p0009
...
% BLOG-PASSAGE: winding-number:p0010
\[
...
\[
  |F(z+h)-F(z)|
  =\left|\int_{\gamma_{z\to z+h}}f(w)\,dw\right|
  \le M\,|\gamma_{z\to z+h}|=M|h|.
\]
Thus, \(F\) is uniformly continuous near \(z_0\), and therefore \(F\) can be continuously extended to \(z_0\).

% BLOG-PASSAGE: winding-number:p0011
...
% BLOG-PASSAGE: winding-number:p0012
Replacing \(f\) with \(g\), we see that \(g(z)=\frac{H(z)-H(z_0)}{z-z_0}\)
is holomorphic on \(U\).

% BLOG-PASSAGE: winding-number:p0013
\textbf{Note.}
Uniform continuity is useful here because the image of a Cauchy sequence is Cauchy: if \(x_n\to x\), then \(f(x_n)\to f(x)\); moreover, if \(y_n\to x\), then \(f(y_n)\to f(x)\). This property helps us extend functions and fill gaps in their domains.

% BLOG-PASSAGE: winding-number:p0014
...
\[
  ...
  \qquad z_0\notin\gamma([0,1]).
\]
The reason we think so is that, given any continuous closed curve (let us assume injectivity for simplicity), we intuitively have an idea of what is ``inside'' \(\gamma\) and what is outside. Let
\[
  \operatorname{dist}(z_0,\gamma):=\inf_t|z_0-\gamma(t)|=d>0.
\]

% BLOG-PASSAGE: winding-number:p0015
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.60\linewidth]{note5-fig-01.pdf}

\end{figure}

% BLOG-PASSAGE: winding-number:p0016
...
\[
  \frac{1}{2\pi i}\int_{\gamma_P}\frac{dz}{z-z_0}=1
\]
...
\[
  ...
  =1\text{ or }0,
\]
based on where \(z_0\) is. There is good reason to believe that this integral captures the notion of interior and exterior. But does this integral also capture the notion of interior and exterior topologically? That is, given any \(z_0\) in the ``interior'' of \(\gamma\), can we also say that there exists \(\varepsilon>0\) such that \(B_\varepsilon(z_0)\) is also in the ``interior'' of \(\gamma\)? The answer is affirmative here too.

% BLOG-PASSAGE: winding-number:p0017
If we define
\[
  \gamma'(t)=\gamma(t)-h,
...
% BLOG-PASSAGE: winding-number:p0018
...
% BLOG-PASSAGE: winding-number:p0019
\paragraph{Proof 1 (geometric).}
Approximate \(\gamma\) with a polygonal path \(\gamma_P\) such that \(W_\gamma(z_0)=W_{\gamma_P}(z_0).\)
This \(\gamma_P\) can be decomposed as a concatenation of simple closed polygonal paths. Thus, \(\gamma_P=\gamma_{P_1}+\gamma_{P_2}+\cdots+\gamma_{P_n},\)
...
\[
  \frac{1}{2\pi i}\int_{\gamma_{P_i}}\frac{dz}{z-z_0}=\pm1\text{ or }0,
\]
depending on the orientation of \(\gamma_{P_i}\) and the position of \(z_0\).

% BLOG-PASSAGE: winding-number:p0020
...
\]
As \(\gamma(0)=\gamma(1)\), \(e^{-g(1)}=1\), and therefore \(g(1)\in2\pi i\mathbb Z\). Hence \(g(1)/(2\pi i)\) is an integer.

% BLOG-PASSAGE: winding-number:p0021
...
% BLOG-PASSAGE: winding-number:p0022
\begin{figure}[H]
  \centering
  \begin{minipage}[b]{.47\linewidth}
    \centering
    \includegraphics[width=0.96\linewidth]{note5-fig-02.pdf}

  \end{minipage}
  \begin{minipage}[b]{.47\linewidth}
    \centering
    \includegraphics[width=0.96\linewidth]{note5-fig-03.pdf}

  \end{minipage}
\end{figure}

% BLOG-PASSAGE: winding-number:p0023
...
\[
  ...
  \{z\mid W_\gamma(z)=0\}.
\]
Assume that \(\gamma\) is a simple closed curve. The essential question we now ask is: how many components does \(\mathbb C\setminus\gamma([0,1])\) have? We know that there are at least two. The \emph{Jordan Curve Theorem} states that there are exactly two. That is, the sets
\[
  \{z\mid W_\gamma(z)=1\}
  \quad\text{and}\quad
  \{z\mid W_\gamma(z)=0\}
...
% BLOG-PASSAGE: winding-number:p0024
...
% BLOG-PASSAGE: winding-number:p0025
...
% BLOG-PASSAGE: winding-number:p0026
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.64\linewidth]{note5-fig-04.pdf}

\end{figure}

% BLOG-PASSAGE: winding-number:p0027
Now let
\[
...
% BLOG-PASSAGE: winding-number:p0028

Therefore, \(\operatorname{int}(\gamma_P)\subset\operatorname{int}(\gamma).\)
Now take any point in \(\operatorname{int}(\gamma_P)\), say \(z_0\). A straight-line contraction is not available when the polygon is concave, so the concave vertices must first be removed. To do that, one must ask for a classic theorem in polygon theory.

% BLOG-PASSAGE: winding-number:p0029
\subsection*{Two-Ears Theorem}
An ear of a polygon is a vertex for which the segment joining its two neighboring vertices lies inside the polygon. The theorem states that every simple polygon has at least two ears.

% BLOG-PASSAGE: winding-number:p0030
...
% BLOG-PASSAGE: winding-number:p0031
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.64\linewidth]{note5-fig-05.pdf}

\end{figure}

% BLOG-PASSAGE: winding-number:p0032
The homotopy
\[
...
% BLOG-PASSAGE: winding-number:p0033
Now assume it is true for all polygons with fewer than \(N\) edges. Let \(\gamma_P\) be a polygon with \(N\) edges. We know all polygons with fewer than \(N\) edges can be deformed to any one of their edges. Using the Two-Ears Theorem, split the polygon into two, \(\gamma_{P'}\) and a triangle, with \(\gamma_{a\to b}\) the common edge. Contract \(\gamma_{P'}\) to the edge \(\gamma_{a\to b}\), and apply the same argument to the triangle. If \(H(s,t)\) and \(H'(s,t)\)

% BLOG-PASSAGE: winding-number:p0034

denote these homotopies, then \(H(s,t)+H'(s,t)\) is the required one.

% BLOG-PASSAGE: winding-number:p0035
Therefore, polygons are null-homotopic because each segment is a degenerate convex set and hence null-homotopic.

% BLOG-PASSAGE: winding-number:p0036
...
% BLOG-PASSAGE: winding-number:p0037
...
% BLOG-PASSAGE: winding-number:p0038
Here is how these deformations look:

% BLOG-PASSAGE: winding-number:p0039
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.94\linewidth]{note5-fig-06.pdf}

\end{figure}

% BLOG-PASSAGE: winding-number:p0040
...
% BLOG-PASSAGE: winding-number:p0041
\textit{Proof.}
The same separation argument used above for $\gamma_P$ and $\gamma$ applies here. We have
\[
...
% BLOG-PASSAGE: winding-number:p0042
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.64\linewidth]{note5-fig-07.pdf}

\end{figure}

% BLOG-PASSAGE: winding-number:p0043
...
% BLOG-PASSAGE: winding-number:p0044
...
% BLOG-PASSAGE: winding-number:p0045

\textit{Proof.}
Let us differentiate \(W_\gamma(z_0)\).

% BLOG-PASSAGE: winding-number:p0046
...
\]
This contradicts local constancy. Therefore, \(P(z)\) has zeros on a large enough disc.

% BLOG-PASSAGE: winding-number:p0049
\subsection*{Higher-Order Cauchy Integral Formula}
\[
  W_\gamma(z_0)H^{(n)}(z_0)
...
\[
  \frac{d}{dz_0}\bigl(W_\gamma(z_0)H(z_0)\bigr)
...
% BLOG-PASSAGE: winding-number:p0050
\textbf{Remark.}
...
\[
  P(z)=az^2+bz+c.
\]
After an affine change of variable and rescaling, the quadratic has the form \(P(z)=z^2+a\), where $a>0$.
Therefore,
\[
  \mathbb R(\sqrt{-a})
  ...
  \cong\mathbb C.
\]
Thus, \(x^2+1\) is the natural choice. The resulting quadratic extension is unique up to isomorphism.

% BLOG-PASSAGE: winding-number:p0051
