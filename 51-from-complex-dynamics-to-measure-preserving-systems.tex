...
One motivation for ergodic theory comes from complex dynamics. Consider
\[
...
\]
$F(f)$ is called the \emph{Fatou set} of $f$; it is open by definition.
...
Studying the dynamics of $f$ includes studying stable and chaotic sets (or Fatou and Julia sets).
...
% Source page 4
Since $\varphi$ is only a change of coordinates on the surface, the dynamics should remain unchanged. Indeed:
...
This motivates the focus on critical and fixed points. Zeros and poles of $f\colon\widehat{\mathbb C}\to\widehat{\mathbb C}$
...
\]
then the zeros and poles are sent to the points $a/c$, $b/d$ on the
Riemann sphere.
\begin{figure}[H]
  \centering
...
coordinates. This choice is useful in analysis but irrelevant to the underlying dynamics. A useful strategy is to choose coordinates in which the function is simpler.
In particular, if $U\subset S$ and there is
$\varphi\colon U\to\varphi(U)\subseteq\mathbb C$, we seek a $\varphi$ that makes
...
Ergodic theory offers many qualitative answers to such questions. Here we
develop the parts of the theory that lead towards Szemer\'edi's theorem,
beginning with measure-preserving systems.
