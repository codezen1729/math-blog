...
By the pigeonhole principle, if $|\Delta(P)|=1$, there must either be a
square or an equilateral triangle in the configuration, and in either case
$|\Delta(P)|\ge2$.
% Author check: the pigeonhole argument is reproduced as written.
...
\[
 ...
 \longrightarrow\mathbb R,\qquad
 F(p_1,\ldots,p_n):=\prod_{1\le\ell\le i<j<k\le n}
 \bigl(|p_i-p_j|-|p_k-p_\ell|\bigr).
\]
% Author check: the unusual product indexing is retained from the source.
$F$ is nonzero on a dense open set in $\mathbb R^{2n}$. On that dense open
set, some sets of points $\{p_1,p_2,\ldots,p_n\}$ need not all be distinct.
Regardless, for all configurations $P$ such that $|P|\le n$,
$|\Delta(P)|=\binom{|P|}{2}$.
% Author check: the quantifier and distinctness assertions are as in the source.
...
Indeed, if $P=\{(0,1),(1,1),\ldots,(n-1,1)\}$, then $|\Delta(P)|=n$.
Thus $g(n)\le n$ trivially, but Erd\H{o}s was able to beat $n$ by
considering the following clever arrangement.
% Author check: the source counts n distances in this n-point configuration.
He considered an integer grid of size $\sqrt N\times\sqrt N$, as shown below:
$[0,\sqrt N]\times[0,\sqrt N]\cap\mathbb Z^2$.
\begin{figure}[H]
...
We have
\[
 \Delta(P)=\{\sqrt A:A=x^2+y^2,\ 0\le x,y\le\sqrt N\}
 \quad\Longrightarrow\quad 0\le A\le2N.
\]
Notice that $|\Delta(P)|=|\{0\le n\le2N:n\text{ is a sum of two squares}\}|$.
Therefore
\[
 g(n)\le\bigl|\{0\le k\le2n:k\text{ is a sum of two squares}\}\bigr|.
\]
...
sketch an estimate for $r_2(m)$.
% Author check: the notation switches from pi_2 to r_2 in the source.
Note that, for $p\equiv3\pmod4$, if $2\mid\nu_p(m)$, then $p<\sqrt m$
(because $p^2\mid m$). From the integers up to $m$, we must remove all odd
powers of $p$ and their multiples, where $p\equiv-1\pmod4$. Note that this
is equivalent to combining all the even powers of $p$ and their multiples,
where $p\equiv-1\pmod4$. Let us do that via inclusion--exclusion!
% Author check: missing divisibility hypotheses and the strict inequality are retained.
Given a prime $p\le m$, the quantity $m(1-1/p)$ is roughly the number of
integers at most $m$ not divisible by $p$. Similarly, $m/p^2$ is the number
of multiples of $p^2$ at most $m$. Hence the number of $k$ such that
$p\nmid k$ or $p^2\mid k$ is
\[
 m\left(1-\frac1p\right)\left(1+\frac1{p^2}\right)+\frac m{p^3},
 \qquad
 m\left(1-\frac1p\right)\left(1+\frac1{p^2}\right)
 =m-\frac mp+\frac m{p^2}-\frac m{p^3}.
\]
% Source page 5
One extends the inclusion--exclusion principle and ends up with
\[
...
But let us ignore that, for $P(k)<k^2$ and $\sum_k k^2/p^k$ converges.
% Author check: the product heuristic and the partition-function bound are retained.
Now, if $p\mid m$ and $2\mid\nu_p(m)$, then $p\le\sqrt m$. Thus
\[
 r_2(m)\approx
 m\prod_{\substack{p\le\sqrt m\\p\equiv-1\ (4)}}\left(1-\frac1p\right)
 \prod_{\substack{p^2\le\sqrt m\\p\equiv-1\ (4)}}\left(1+\frac1{p^2}\right)
 \cdots.
\]
Each product after the first converges, so
$r_2(m)\approx m\prod_{p\le\sqrt m,\ p\equiv-1\ (4)}(1-1/p)$.
One can now prove that
\[
 \prod_{\substack{p\le\sqrt m\\p\equiv-1\ (4)}}\left(1-\frac1p\right)
...
numbers and is approximately $m/\log m$. Hence, by the Prime Number Theorem,
$\prod_{p\le\sqrt m}(1-1/p)\approx1/\log m$.
Now, as primes are equidistributed modulo $4$ between $1$ and $-1$,
\[
 \prod_{\substack{p\le\sqrt m\\p\equiv1\ (4)}}\left(1-\frac1p\right)
 \approx
 \prod_{\substack{p\le\sqrt m\\p\equiv-1\ (4)}}\left(1-\frac1p\right)
...
\[
 g(n)\le r_2(2n)\approx\frac{2n}{\sqrt{\log(2n)}}
...
lower bound roughly of order $O(n^{1-\varepsilon})$ must exist for every
...
\[
 \limsup_{x\to\infty}|g(x)/x^c|>0.
\]
\end{conjecture}
% Author check: the source's O notation for a lower bound and limsup definition are retained.
...
Draw $t$ circles centred at $p_0$ such that every point in $P$ belongs to
one such circle and each circle has at least one point of $P$.
% Source page 8
By the pigeonhole principle, at least one circle must have at least $n/t$
points of $P$. Again by the pigeonhole principle, at least one of the
hemispheres must have $n/(2t)$ points. Pick the rightmost point to see that
it will have at least $n/(2t)$ distinct distances to points of $P$ on that
hemisphere. Therefore
\[
 |\Delta(P)|\ge\max\left(t,\frac n{2t}\right)
 \ge\sqrt{t\left(\frac n{2t}\right)}=\sqrt{\frac n2},
 \qquad g(n)\ge\sqrt{\frac n2}.
\]
Thus the constant $c$ here is $1/\sqrt2$.
% Author check: the source includes the centre in its point count and uses "hemisphere".
\end{proof}
\begin{remark}
...
\begin{figure}[H]
 \centering\includegraphics[width=.64\textwidth]{m2-fig-03.pdf}
...
\[
 ...
 \qquad 3k'\le\ell.
\]
% Author check: the source writes 3k' <= ell here, without defining k'.
...
\]
Then $A_i=\bigcup_{k'}(A_i\cap B_{k'})$, since every point in $A_{3j}$
at distance $d_i$ from $x$ is at some distance from $y$.
% Source page 10
...
\]
Note that $A_i$ and $B_{k'}$ are points on the circles above $L$, so
$|A_i\cap B_{k'}|\le1$, giving $k\ge\sqrt{n_j}$.
In the $j$th annulus $\widetilde A_{3j}$ there are therefore at least
$\sqrt{n_j}$ distinct distances to $x$.
Notice that $\widetilde A_{3j}$ and $\widetilde A_{3j+3}$ can never have
two points at the same distance, so we can sum them together to obtain
$|\Delta(P)|\ge\sum_j\sqrt{n_j}$.
% Author check: the source infers a bound for distances to x from the union of distances to x and y.
...
\]
Also, $|\Delta(P)|\ge\max_j n_j$. To see this, pick the annulus
$\widetilde A_{3j}$ in which $n_j$ is maximum, and in it pick the rightmost
point, just as we did in the proof of Erd\H{o}s's theorem. We obtain
\[
 |\Delta(P)|\ge\max_j n_j,\qquad
 |\Delta(P)|\ge\frac n{6\sqrt{\max_j n_j}}.
\]
% Author check: the rightmost-point argument is preserved for annuli as written.
% Source page 11
It follows that
\[
 |\Delta(P)|^2\,|\Delta(P)|
