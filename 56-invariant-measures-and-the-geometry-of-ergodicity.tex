...
\]
is open and dense in $C(X)$. Thus $\bigcap_{i,j}E_{ij}$ is nonempty, as $C(X)$ is a Baire space. Therefore, if $\mathcal M^T(X)$ were the convex hull of finitely many ergodic extreme points, then the state space $X$ would have a finite decomposition into ergodic components.
