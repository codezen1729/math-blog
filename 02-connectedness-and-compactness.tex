...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=.68\linewidth]{note1-fig-01.pdf}
  \textit{Figure N1.1}
\end{figure}

It follows that, if $f\in L^1(\mu)$, then
\[
...
There is no continuous function $f\colon\mathbb C^*\to\mathbb C^*$ such that \(f(z)^2=z\qquad(\forall z\in\mathbb C^*).\)
(CMI Entrance)
...
for a domain $\Omega\subset\mathbb C\setminus\{0\}$ if and only if $\{0\}$ and $\{\infty\}$ belong to different components of $\Omega^c$.

\textbf{Exercise.}
Observe that the preceding comment motivates the general version, and see why the earlier result is true assuming this general version.
...
Since $1$ is a limit point, let $v\in S^1\setminus G$. There is a $k'$ with
\[
...
\textbf{Remark.}
As a warning, one must quickly realize that Heine--Borel fails even for an infinite discrete space: the whole space is both closed and bounded, but it is not compact.
...
and $z_0$ belongs to a


box of diameter less than $\varepsilon$. Hence $z_0$ is a limit point, so $z_0\in G$ and $G=\mathbb C$.
...
\]
Now, if \(\left\langle re^{i\theta}:re^{i\theta}\in G\right\rangle\)
is a connected closed subgroup of $\mathbb C^*$, then
\[
\left\langle \log r+i\theta:re^{i\theta}\in G\right\rangle
\]
is a connected closed subgroup of $(\mathbb C,+)$. To see this, all one has to observe is that
\[
f\colon\mathbb R_+\times[0,2\pi)\longrightarrow\mathbb C,
\qquad f(r,\theta)=\log r+i\theta,
\]
