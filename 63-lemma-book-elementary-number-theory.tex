...
% BLOG-CONTENT-BEGIN
\subsection{Cover inscriptions}
The notebook cover records the following formulas:
\[
1+2+3+4+\cdots=-\frac1{12},\qquad
1+\frac14+\frac19+\frac1{16}+\frac1{25}+\cdots=\frac{\pi^2}{6},
\]
\[
1-1+1-1+1-1+\cdots=\frac12,\qquad
...
\qquad x^2\geq0.
\]
The partially cropped lower edge also contains $e^{i\pi}=-1$ and
...
The notes write $n\equiv1\pmod{2^{\gamma_i}}$, hence $n-1=2^{\gamma_i}t$.
...
$n\nmid\sum_{r=1}^{n-3}r(r!)$ if and only if $n$ is prime.
Note that
\[
\sum_{r=1}^{n-3}r(r!)=\sum_{r=1}^{n-3}\bigl((r+1)!-r!\bigr)=(n-2)!-1.
\]
The notes conclude $n\nmid(n-2)!-1\Rightarrow n$ is prime.
% Author check: divisibility signs and the claimed equivalence are retained from the source.
...
There are many such factorization techniques that do not look elementary to the eye.
% BLOG-CONTENT-END
\end{document}
