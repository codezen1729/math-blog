\documentclass[11pt,a4paper]{article}
\input{styles/blog-preamble.tex}
\graphicspath{{figures/complex-analysis/}}
\begin{document}
\section*{Goursat's Theorem and Cauchy's Theorem}
% BLOG-CONTENT-BEGIN
% BLOG-PASSAGE: goursat-cauchy:p0001
\subsection*{Tool 8 — Linear Approximation}
If \BlogPost{complex-differentiation}{$f$ is differentiable at $z_0$}, then we have
\[
  f(z)=f(z_0)+f'(z_0)(z-z_0)+\psi(z),
  \qquad \psi(z)=o(z-z_0).
\]

% BLOG-PASSAGE: goursat-cauchy:p0002

\subsection*{Goursat's Theorem}
Let $T\subset U$ be the convex hull of $z_1,z_2,z_3$.
Then, if $f\colon U\to\mathbb C$ is holomorphic,
\[
  \int_{\partial T} f(z)\,dz=0,
\]
where $\partial T$ is the polygonal path
$\gamma_{z_1\to z_2\to z_3\to z_1}$.

% BLOG-PASSAGE: goursat-cauchy:p0003
\textit{Proof.}

% BLOG-PASSAGE: goursat-cauchy:p0004
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.58\textwidth]{note3-fig-01.pdf}

\end{figure}

% BLOG-PASSAGE: goursat-cauchy:p0005
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.56\textwidth]{note3-fig-02.pdf}

\end{figure}

% BLOG-PASSAGE: goursat-cauchy:p0006
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.64\textwidth]{note3-fig-03.pdf}


\end{figure}

% BLOG-PASSAGE: goursat-cauchy:p0007

As the picture suggests,
\[
  \left|\int_{T_0}f(z)\,dz\right|
  \leq
  4\sup_{i\in\{1,2,3,4\}}
  \left|\int_{T_i}f(z)\,dz\right|,
\]
and, further, we can repeat the argument for $T_i$ (say $T_1$, without loss of
generality) to obtain
\[
  \left|\int_{T_0}f(z)\,dz\right|
  \leq 4\left|\int_{T_1}f(z)\,dz\right|
  \leq 4^2\left|\int_{T_2}f(z)\,dz\right|
  \leq\cdots\leq
  4^n\left|\int_{T_n}f(z)\,dz\right|
\]
after $n$ stages. Now one has to compensate for the $4^n$.
Note first that the $T_n$, seen as solid triangles, are compact sets and
$T_n\supseteq T_{n+1}$, so
\[
  \bigcap_n T_n=\{z_0\}
\]
for some $z_0$ in the solid triangle $T_0$. Now let us approximate $f$ around
$z_0$:
\[
  f(z)=f(z_0)+f'(z_0)(z-z_0)+\psi(z)(z-z_0),
  \qquad \psi(z)\to0\quad\text{as }z\to z_0.
\]
Note that $f(z_0)$ and $f'(z_0)(z-z_0)$ have primitives. Therefore, we
essentially have
\[
  \int_{T_n}f(z)\,dz
  =\int_{T_n}\psi(z)(z-z_0)\,dz.
\]
Let $d_n$ be the diameter of $T_n$ and $p_n$ its perimeter. Then
\[
  d_n=\frac{d_0}{2^n},
  \qquad
  p_n=\frac{p_0}{2^n},
\]
and hence
\[
  \left|\int_{T_n}\psi(z)(z-z_0)\,dz\right|
  \leq \int_{T_n}|\psi(z)(z-z_0)|\,|dz|
  \leq \frac{\varepsilon_n d_0p_0}{4^n},
  \qquad
  \varepsilon_n=\sup_{z\in T_n}|\psi(z)|.
\]
Thus the error in the linear approximation helps us compensate for the $4^n$:
\[
  \left|\int_{T_0}f(z)\,dz\right|
  \leq 4^n\left|\int_{T_n}f(z)\,dz\right|
  \leq 4^n\left(\frac{\varepsilon_n d_0p_0}{4^n}\right)
  \leq \varepsilon_n d_0p_0.
\]
Let $n\to\infty$. Since $\varepsilon_n\to0$,
\[
  \int_{T_0}f(z)\,dz=0.
\]

% BLOG-PASSAGE: goursat-cauchy:p0008
\textbf{Remark.}
Holomorphicity at every point is unnecessary here. Suppose that $f$ is merely
bounded near a point $z_0$ in the solid triangle. By subdividing the triangle
as illustrated below, one may reduce to the case in which $z_0$ is a vertex.

% BLOG-PASSAGE: goursat-cauchy:p0009
\begin{figure}[H]
  \begin{minipage}[t]{.48\textwidth}
    If $z_0$ is in the interior of $T_0$,\par
    \begin{minipage}[c][5.1cm][c]{\linewidth}
      \centering
      \includegraphics[width=0.92\linewidth]{note3-fig-04.pdf}
    \end{minipage}


  \end{minipage}
  \begin{minipage}[t]{.48\textwidth}
    If $z_0$ is on $T_0$,\par
    \begin{minipage}[c][5.1cm][c]{\linewidth}
      \centering
      \includegraphics[width=0.92\linewidth]{note3-fig-05.pdf}
    \end{minipage}


  \end{minipage}
\end{figure}

% BLOG-PASSAGE: goursat-cauchy:p0010
Now we know that $z_0$ is a vertex:
\begin{figure}[H]
  \begin{minipage}[b]{.48\textwidth}
    \begin{minipage}[c][5.0cm][c]{\linewidth}
      \centering
      \includegraphics[width=0.92\linewidth]{note3-fig-06.pdf}
    \end{minipage}

  \end{minipage}
  \begin{minipage}[b]{.48\textwidth}
    \begin{minipage}[c][5.0cm][c]{\linewidth}
      \centering
      \includegraphics[width=0.92\linewidth]{note3-fig-07.pdf}
    \end{minipage}

  \end{minipage}
\end{figure}

% BLOG-PASSAGE: goursat-cauchy:p0011
For all $a,b\neq z_0$, on $T_1$ and $T_2$ the integral will vanish, and since
$f$ is bounded near $z_0$, the integral on $T_3$ can be made arbitrarily small,
as the perimeter can be made very small. Also note that what we essentially
need to prove Goursat's Theorem is a linear form as given below. For every
$z_0$,
\[
  f(z)=a_{z_0}+b_{z_0}(z-z_0)+\psi_{z_0}(z)(z-z_0)
  \qquad\text{on }U\setminus\{z_0\},
\]
where $\psi_{z_0}(z)\to0$ as $z\to z_0$.

% BLOG-PASSAGE: goursat-cauchy:p0012
Observe immediately that holomorphic functions except on a discrete set, where
they have removable discontinuities, are candidates. Thus Goursat's Theorem
works for a wider class of functions than merely holomorphic functions.

% BLOG-PASSAGE: goursat-cauchy:p0013

\subsubsection*{Application — Holomorphic primitives on convex domains}
Let $f$ be holomorphic on an open, connected, convex set.
Then $f$ has a primitive; that is, there exists $F$, holomorphic on the same
convex domain, such that $F'=f$.

% BLOG-PASSAGE: goursat-cauchy:p0014
\textit{Proof.}
Let $\Omega$ be the domain, and fix $z_0\in\Omega$. Define
\[
  F(z):=\int_{\gamma_{z_0\to z}}f(w)\,dw.
\]

% BLOG-PASSAGE: goursat-cauchy:p0015
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.64\textwidth]{note3-fig-08.pdf}

\end{figure}

% BLOG-PASSAGE: goursat-cauchy:p0016
By Goursat's Theorem, we have
\[
  F(z+h)-F(z)=\int_{\gamma_{z\to z+h}}f(w)\,dw.
\]
For $f$ at $z$, we have
\[
  f(w)=f(z)+f'(z)(w-z)+\psi(w),
  \qquad \psi(w)=o(w-z).
\]
Therefore,
\begin{align*}
  F(z+h)-F(z)
  &=\int_{\gamma_{z\to z+h}}f(z)\,dw
    +\int_{\gamma_{z\to z+h}}f'(z)(w-z)\,dw
    +\int_{\gamma_{z\to z+h}}\psi(w)\,dw\\
  &=f(z)h+f'(z)\left(\frac{w^2}{2}-wz\right)\Big|_z^{z+h}
    +\int_{\gamma_{z\to z+h}}\psi(w)\,dw\\
  &=f(z)h+f'(z)\left(
      \frac{(z+h)^2}{2}-(z+h)z-\left(\frac{z^2}{2}-z^2\right)
    \right)
    +\int_{\gamma_{z\to z+h}}\psi(w)\,dw\\
  &=f(z)h+\frac{f'(z)h^2}{2}
    +\int_{\gamma_{z\to z+h}}\psi(w)\,dw.
\end{align*}
This gives a second-order Taylor approximation for $F$. Note that
\[
  \left|\int_{\gamma_{z\to z+h}}\psi(w)\,dw\right|
  \leq |h|\sup_{w\in\gamma_{z\to z+h}}|\psi(w)|,
\]
which goes to $0$ as $h\to0$. Hence
\[
  \lim_{h\to0}\frac{F(z+h)-F(z)}{h}=f(z).
\]

% BLOG-PASSAGE: goursat-cauchy:p0017

\subsection*{Tool 9 — Complex Tube Lemma}
Given any continuous curve in a domain $\Omega$, there exists a polygonal path
``close'' to the curve.

% BLOG-PASSAGE: goursat-cauchy:p0018

\textbf{Note.}
These polygons are useful, for one can invoke Goursat's Theorem. In fact, we can
say something more: if there is a convex set $U\subset\Omega$ such that the
continuous curve and its polygonal ``approximation'' both lie in $U$, then the
polygonal path and the curve form a closed continuous curve in $U$. If,
moreover, we demand that $\gamma$ be rectifiable, then
\[
  \int_\gamma f(z)\,dz=\int_P f(z)\,dz
  \qquad\bigl(\forall f\in H(\Omega)\bigr),
\]
where $P$ is the polygonal path. This is because $f$ has a primitive in $U$, as
was seen previously.

% BLOG-PASSAGE: goursat-cauchy:p0019
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.68\textwidth]{note3-fig-09.pdf}

\end{figure}

% BLOG-PASSAGE: goursat-cauchy:p0020
\textit{Proof.}
Let $\gamma\colon[0,1]\to\Omega$ be the curve. Then
$\gamma|_{[0,1]}$ is compact. For every $z\in\gamma|_{[0,1]}$, there exists
$\varepsilon_z$ such that $B_{\varepsilon_z}(z)\subset\Omega$, and
\[
  \gamma|_{[0,1]}\subseteq\bigcup_z B_{\varepsilon_z}(z).
\]
This has a finite cover. Let $\{U_j\}_{j=1}^n$ be the finite balls that cover
$\gamma|_{[0,1]}$. If it is not already present, add the ball
$B_{\varepsilon_{\gamma(0)}}(\gamma(0))$ to the finite collection.

% BLOG-PASSAGE: goursat-cauchy:p0021
Here is a first attempt at arranging those balls. As $[0,1]$ is connected, every open set in the finite cover will have nonempty intersection with at least one open set other than itself in the finite cover. Let $B_{\varepsilon_{\gamma(i)}}(\gamma(i))$ be such that
\[
  B_{\varepsilon_{\gamma(i)}}(\gamma(i))
  \cap B_{\varepsilon_{\gamma(0)}}(\gamma(0))\neq\varnothing.
\]
Let $i$ be the smallest value in $[0,1]$ for which the first ball centered at $\gamma(i)$ is in $\{U_j\}$ and
\[
B_{\varepsilon_{\gamma(i)}}(\gamma(i))
\cap B_{\varepsilon_{\gamma(0)}}(\gamma(0))\neq\varnothing.
\]
Note that, as $\gamma|_{[0,i]}$ is connected,
\[
  \gamma|_{[0,i]}
  \subset
  B_{\varepsilon_{\gamma(0)}}(\gamma(0))
  \cup B_{\varepsilon_{\gamma(i)}}(\gamma(i));
\]
otherwise, the minimality would be contradicted. Arrange
$U_{i_m}=B_{\varepsilon_{\gamma(i_m)}}(\gamma(i_m))$ so that minimality is demanded consecutively; that is,
\[
  \gamma|_{[i_m,i_{m+1}]}
  \subset
  B_{\varepsilon_{\gamma(i_m)}}(\gamma(i_m))
  \cup
  B_{\varepsilon_{\gamma(i_{m+1})}}(\gamma(i_{m+1})),
\]
where $\{i_m\}$ partitions $[0,1]$. Again, if
$B_{\varepsilon_{\gamma(1)}}(\gamma(1))$ does not belong to $\{U_j\}$, just add it. Now $\gamma_{\gamma(0)\to\gamma(i_1)\to\cdots\to\gamma(1)}$ is the required polygonal path. The following figure illustrates the idea in the proof.

% BLOG-PASSAGE: goursat-cauchy:p0022
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.70\textwidth]{note3-fig-10.pdf}
\end{figure}

% BLOG-PASSAGE: goursat-cauchy:p0023
Thus there exists
$B_{\varepsilon_{\gamma(j)}}(\gamma(j))$, with $j<i$, such that
\[
  B_{\varepsilon_{\gamma(j)}}(\gamma(j))
  \cap B_{\varepsilon_{\gamma(0)}}(\gamma(0))\neq\varnothing.
\]
One can ``see'' the polygonal paths here. After some stage we will have
\[
  B_{\varepsilon_{\gamma(i_m)}}(\gamma(i_m))
  \cap B_{\varepsilon_{\gamma(1)}}(\gamma(1))\neq\varnothing.
\]
We stop there, piecewise. One can observe here that, by construction, $\gamma_p$ and $\gamma$ are piecewise homotopic.

% BLOG-PASSAGE: goursat-cauchy:p0024
\textbf{Correction to the preceding minimality argument.}
The minimality argument as presented will not work. One must tweak it.
Let
\[
  i=\inf\left(
    \gamma^{-1}\left(
      \partial B_{\varepsilon_{\gamma(0)}}(\gamma(0))
      \cap\gamma|_{[0,1]}
    \right)
  \right).
\]
Since
$\partial B_{\varepsilon_{\gamma(0)}}(\gamma(0))\cap\gamma|_{[0,1]}$
is closed, we would have
\[
  \gamma(i)\in
  \partial B_{\varepsilon_{\gamma(0)}}(\gamma(0))
  \cap\gamma|_{[0,1]}.
\]
Choose, among the $\{U_j\}$, a ball that covers $\gamma(i)$; let that be
$B_{\varepsilon_{\gamma(j)}}(\gamma(j))$. Note that
\[
  \gamma|_{[0,i)}\subset B_{\varepsilon_{\gamma(0)}}(\gamma(0)).
\]
If not, there exists $j<i$ such that
$\gamma(j)\notin B_{\varepsilon_{\gamma(0)}}(\gamma(0))$, and, for
\[
  B_{\varepsilon_{\gamma(0)}}(\gamma(0))\cap\gamma|_{[0,j]}
  \quad\text{and}\quad
  \overline{B_{\varepsilon_{\gamma(0)}}(\gamma(0))}^{\,c}
  \cap\gamma|_{[0,j]}
\]
not to be a separation of $\gamma|_{[0,j]}$, there exists $l<j<i$ such that
$\gamma(l)\in\partial B_{\varepsilon_{\gamma(0)}}(\gamma(0))$, thereby
contradicting minimality. Now we have
\[
  \gamma|_{[0,i]}
  \subset
  B_{\varepsilon_{\gamma(0)}}(\gamma(0))
  \cup B_{\varepsilon_{\gamma(j)}}(\gamma(j)).
\]
Proceed.

% BLOG-PASSAGE: goursat-cauchy:p0025
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.92\textwidth]{note3-fig-11.pdf}
\end{figure}

% BLOG-PASSAGE: goursat-cauchy:p0026
Now it is an easy exercise to note that, if $\gamma$ is rectifiable, the
polygonal path we obtained (denote it by $\gamma_p$) yields
\[
  \int_\gamma f(z)\,dz=\int_{\gamma_p}f(z)\,dz.
\]
Note the $\langle a_i\rangle$ and $\langle b_i\rangle$ marked in the figure
above. Denote by $\operatorname{Tube}(\gamma)$ the quantity
\[
  \min_i|a_i-b_i|.
\]

% BLOG-PASSAGE: goursat-cauchy:p0027
\textbf{Exercise.}
Construct a ``tube'' of ``width'' $\operatorname{Tube}(\gamma)$ around
$\gamma$ in $\Omega$.

% BLOG-PASSAGE: goursat-cauchy:p0028
\subsection*{Cauchy's Theorem}
Let $f\in H(U)$, where $U$ is a domain, and let $\gamma_0$ and $\gamma_1$ be
rectifiable and homotopic in $U$. Then
\[
  \int_{\gamma_0}f(z)\,dz=\int_{\gamma_1}f(z)\,dz.
\]

% BLOG-PASSAGE: goursat-cauchy:p0029
\textit{Proof.}
By compactness of the homotopy
$\gamma\colon[0,1]\times[0,1]\to U$, there exists $r>0$ such that
\[
  B_r(\gamma(s,t))\subset U
  \qquad\forall(s,t)\in[0,1]\times[0,1].
\]
The map $\gamma$ is also uniformly continuous. Hence, whenever
$|s-s'|<\delta$ and $|t-t'|<\delta$, \(|\gamma(s',t')-\gamma(s,t)|<\frac r4.\)
With these two observations, the proof is in pictures.

% BLOG-PASSAGE: goursat-cauchy:p0030
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.96\textwidth]{note3-fig-12.pdf}
\end{figure}

% BLOG-PASSAGE: goursat-cauchy:p0031
Let
\[
  C_{ij}:=
  \gamma_{\gamma(s_i,t_{j-1})\to\gamma(s_i,t_j)
  \to\gamma(s_{i-1},t_j)\to\gamma(s_{i-1},t_{j-1})
  \to\gamma(s_i,t_{j-1})}.
\]
Choose the stages $s_i$ such that $\gamma(s_{i+1},t)$ is in the tube of width
$\operatorname{Tube}(\gamma(s_i,t))$. This will ensure that these polygonal
paths are all in $\Omega$; that is, one can find an ``approximate'' homotopy or
deformation and carry $\int_{\gamma_0}f(z)\,dz$ via convex sets to
$\int_{\gamma_1}f(z)\,dz$.

% BLOG-PASSAGE: goursat-cauchy:p0032
\textbf{Exercise.}
Fill in the details.

% BLOG-PASSAGE: goursat-cauchy:p0033
It is worth mentioning two important corollaries of Cauchy's Theorem, which are
more useful than the theorem itself.

% BLOG-PASSAGE: goursat-cauchy:p0034
\subsection*{Tool 10 — Cauchy Integral Formula}
\[
  f(z_0)=\frac{1}{2\pi i}\int_{C_R}\frac{f(z)}{z-z_0}\,dz.
\]
Here $f\in H(\Omega)$, and $C_R$ is a circle of radius $R$ such that
$\operatorname{int}C_R\cup C_R\subset\Omega$.

% BLOG-PASSAGE: goursat-cauchy:p0035

\subsection*{Tool 11 — Higher-Order Cauchy Integral Formula}
\[
  f^{(n)}(z_0)=\frac{n!}{2\pi i}
  \int_{C_R}\frac{f(z)}{(z-z_0)^{n+1}}\,dz.
\]

% BLOG-PASSAGE: goursat-cauchy:p0036

\subsection*{Tool 12 — Local Analyticity}
\[
  f(z)=\sum_{n\geq0}\frac{f^{(n)}(z_0)}{n!}(z-z_0)^n
  \qquad\bigl(\forall z_0, z\in\operatorname{int}C_R\bigr).
\]

% BLOG-PASSAGE: goursat-cauchy:p0037
Tool 12 follows from Cauchy's integral formulas, Tools 10 and 11.
\subsubsection*{Application — Consequences of Cauchy's formulas}
The following applications of Tools 10, 11, and 12 may seem
standard, but they record useful consequences worth
recalling.

% BLOG-PASSAGE: goursat-cauchy:p0038

\subsection*{Tool 13 — Identity Theorem}

% BLOG-PASSAGE: goursat-cauchy:p0039
Let $f\in H(\Omega)$. If $z_n\to z_0$ in $\Omega$ and $f(z_n)=0$, then
$f\equiv0$.

% BLOG-PASSAGE: goursat-cauchy:p0040

\textit{Proof.}
Locally around $z_0$,
\[
  f(z)=\sum_n a_n(z-z_0)^n.
\]
Thus
\[
  f(z)=a_m(z-z_0)^m\bigl(1+g(z-z_0)\bigr),
\]
where $m$ is the least index such that $a_m\neq0$. Since
$g(z-z_0)\to0$ as $z\to z_0$, there exists $K$ such that, for every $k\geq K$, \(|g(z_k-z_0)|<1.\)
Therefore,
\[
  a_m(z_k-z_0)^m\neq0
  \quad\text{and}\quad
  1+g(z_k-z_0)\neq0,
\]
but $f(z_k)=0$, a contradiction. Hence such an $m$ does not exist, so
$f(z)\equiv0$ locally. Now note that the set of points
$\{z\mid f(z)=0\}$ is closed. Hence the continuity method would force
$f\equiv0$.

% BLOG-PASSAGE: goursat-cauchy:p0041
\subsection*{Morera's Theorem}
Let $f$ be a continuous function on $\Omega$, and suppose that, for every
closed polygonal path $P$,
\[
  \int_P f(z)\,dz=0.
\]
Then $f\in H(\Omega)$.

% BLOG-PASSAGE: goursat-cauchy:p0042
\textit{Proof.}
Modify the ``existence of a primitive on a convex set'' proof and construct a
primitive for $f$. Let $F'=f$. Since $F\in H(\Omega)$ is locally analytic,
$f$ is locally analytic, and hence $f\in H(\Omega)$.

% BLOG-PASSAGE: goursat-cauchy:p0043
\textbf{Remark.}
Compare this with the Looman--Menchoff Theorem, which just demands the
Cauchy--Riemann equations along with continuity. Which is stronger?

% BLOG-PASSAGE: goursat-cauchy:p0044

\subsubsection*{Application — Polynomial characterization}
Let $f$ be entire such that, for each $z_0$,
$f^{(n)}(z_0)=0$ for some $n$. Then $f$ is a polynomial.

% BLOG-PASSAGE: goursat-cauchy:p0045
\textit{Proof.}
For every $z_0\in\mathbb C$, $f^{(n)}(z_0)=0$ for some $n$. By the
Pigeonhole Principle, for a fixed $n$, $f^{(n)}(z_0)=0$ for uncountably many
$z_0$. Thus the zero set of $f^{(n)}$ has a limit point in some disc, and
hence \(f^{(n)}(z)\equiv0.\)
Therefore, $f$ is a polynomial, because
\[
  f(z)=\sum_{n=0}^{\infty}c_n(z-z_0)^n,
  \qquad
  c_n=\frac{f^{(n)}(z_0)}{n!},
\]
where $z_0$ is a zero of $f$. Hence, for every $m\geq n$, \(\frac{f^{(m)}(z_0)}{n!}=0.\)
Thus $f$ is locally a polynomial, and, by the Identity Theorem, $f$ is globally
a polynomial.

% BLOG-PASSAGE: goursat-cauchy:p0046
\textbf{Remark.}
The local power-series expansion about a point converges up to the
nearest point of singularity.
% BLOG-CONTENT-END
\end{document}
