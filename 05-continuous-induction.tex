...
\]
then $P$ holds for $\mathbb N$. There is a way to extend this to an interval $[0,1]$. Naturally, we expect $P$ to hold at $0$. There are many problems in devising the ``induction'' hypothesis for an interval.
...
for some $\delta$ depending on $t$, then the induction starts. But after the fact that $P$ holds on $[0,\delta)$, if $t\in[0,\delta)$, what if $\delta_t$ is such that
\[
(t,t+\delta_t)\subset[0,\delta)?
\]
The induction would fall dead right there.
...
\end{itemize}
A set being closed is a mathematical way of saying that all limit points of limit points of limit points, and so on, remain in the set: now one can say ``keep going on.'' Then $P$ holds on $[0,1]$.
...
\]
To prove this weakened version, we invoked the fundamental theorem of calculus. Here too we weaken the inequality and prove
\[
...
Let $f\colon U\to\mathbb C$ be continuous. Let $F\colon U\to\mathbb C$ be holomorphic and satisfy $F'=f$. If $\gamma\colon[a,b]\to\mathbb C$ is rectifiable, then
\[
\int_\gamma f(z)\,dz
=F(\gamma(b))-F(\gamma(a)).
\]
...
\int_{\gamma|_{[a,t]}}f(z)\,dz
\]
is \(f(a)(\gamma(t)-\gamma(a))\)
when $a$ and $t$ are close enough. This linear approximation is usually another standard technique, which we have exploited twice now. We might as well present it.
% BLOG-CONTENT-END
\end{document}
