...
% BLOG-CONTENT-BEGIN
% BLOG-PASSAGE: classification-vector-bundles:p0001
...
% BLOG-PASSAGE: classification-vector-bundles:p0002
\begin{proof}
...
% BLOG-PASSAGE: classification-vector-bundles:p0003
...
% BLOG-PASSAGE: classification-vector-bundles:p0004
\underline{\textbf{Classification of Vector Bundles} (over compact Hausdorff spaces)}

% BLOG-PASSAGE: classification-vector-bundles:p0005
...
% BLOG-PASSAGE: classification-vector-bundles:p0006
\begin{remark}
...
% BLOG-PASSAGE: classification-vector-bundles:p0007
\subsection*{Scope of the classification theorem}
Note that the homotopy theorem ensures that the pullback of bundles via maps in $[X, \text{Gr}(n, \mathbb{K}^{\infty})]$ is well-defined. The reason we require $X$ to be compact Hausdorff is largely because such spaces are compactly generated. In particular, with the compact-open topology on mapping spaces, they satisfy:
$$\text{Hom}(X\times Y, Z) \cong \text{Hom}(X, \text{Hom}(Y, Z)).$$
As a result, the hypothesis on $X$ can be weakened to allow paracompact Hausdorff spaces and numerable bundles. Under these hypotheses, the following is a bijective correspondence:
$$[X, \text{Gr}(n, \mathbb{K}^{\infty})] \longrightarrow \text{Vect}_{\mathbb{K}}^n(X)$$
$$[f] \rightarrow [f^*\gamma_n]$$

% BLOG-PASSAGE: classification-vector-bundles:p0008
\begin{proof}
    ...
    \begin{center}
    \underline{FACT}: $\text{Gr}(n, \mathbb{K}^{\infty}) \cong V(n, \mathbb{K}^{\infty})/O_{\mathbb K}(n)$, and it can also be identified with the space of rank-$n$ orthogonal projections on $\mathbb K^\infty$.
    \end{center}
So, $f$ and $g$ can be viewed as families of rank-$n$ orthogonal projections varying continuously over $X$. The fibre isomorphisms may be pictured in the following diagram:
\[
\begin{tikzcd}[row sep=large,column sep=large]
 f^*\gamma_n \arrow[r,"\alpha"] \arrow[d] & g^*\gamma_n \arrow[d] \\
 X \arrow[r,equal] & X.
\end{tikzcd}
\]
Now define
...
$$\big[(v_1,\ldots,v_n),(\lambda_1,\ldots,\lambda_n)\big]\longmapsto
\bigg(\langle v_1,\ldots,v_n\rangle,\sum_i\lambda_i v_i\bigg)$$
...
% BLOG-PASSAGE: classification-vector-bundles:p0009
\begin{remark}
...
% BLOG-PASSAGE: classification-vector-bundles:p0010
\begin{center}
