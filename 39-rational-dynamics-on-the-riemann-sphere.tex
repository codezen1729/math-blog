...
% BLOG-CONTENT-BEGIN
% BLOG-PASSAGE: rational-dynamics-on-the-riemann-sphere:p0001
...
% Source page 2
% BLOG-PASSAGE: rational-dynamics-on-the-riemann-sphere:p0002
...
% BLOG-PASSAGE: rational-dynamics-on-the-riemann-sphere:p0003
...
\end{definition}
% Source page 3
% BLOG-PASSAGE: rational-dynamics-on-the-riemann-sphere:p0004
...
\end{lemma}
\begin{proof}
It is enough to show $z\in F(f)\Longleftrightarrow f(z)\in F(f)$. We first record that postcomposition with $f$ preserves normality. If $\{f^{\circ n}|_U\}_n$ is normal, it is contained in a compact set $K\subset\Hol(U,S)$.
% Source page 4
...
\]
Indeed, $f$ is uniformly continuous on the compact set $\overline{B_\varepsilon(g(C))}$. Thus $\{f^{\circ(n+1)}|_U\}_n$ is normal.

If $f(z)\in F(f)$, there is a neighbourhood $V$ of $f(z)$ on which $\{f^{\circ n}|_V\}_n$ is normal. Since $f$ is open, there is a neighbourhood $U$ of $z$ with $f(U)\subseteq V$. Hence $\{f^{\circ(n+1)}|_U\}_n$ is normal, and adding the single map $\id|_U$ shows that $z\in F(f)$.
...
\end{proof}
% Source page 5
% BLOG-PASSAGE: rational-dynamics-on-the-riemann-sphere:p0005
...
% Source page 6
% BLOG-PASSAGE: rational-dynamics-on-the-riemann-sphere:p0006
...
Conversely, if $\widehat p$ is attracting, there is $\varepsilon>0$ and an $n\ge1$ such that $f^{\circ n}(\mathbb D_\varepsilon)$ is relatively compact in $\mathbb D_\varepsilon$. By Schwarz's lemma, $|(f^{\circ n})'(0)|<1$, so $|f'(0)|<1$.
\end{proof}
Notice that we need holomorphicity of $f$ to conclude that $|\lambda|<1$ is geometrically attracting.

% BLOG-PASSAGE: rational-dynamics-on-the-riemann-sphere:p0007
\paragraph{Repulsion: multiplier $|\lambda|>1$.}
This is usually called topological repulsion. Let $f(\widehat p)=\widehat p$. The point $\widehat p$ is repelling if there is a neighbourhood $U$ of $\widehat p$ such that, for every $p\in U\setminus\{\widehat p\}$, there is $N_p$ with $f^{\circ n}(p)\notin U$ for some $n\ge N_p$. Equivalently, $f$ is locally invertible at $\widehat p$ and $\widehat p$ is attracting for the local inverse fixing $\widehat p$. In other words, all orbits except that of $\widehat p$ eventually escape $U$.
\begin{figure}[H]\centering
...
\end{lemma}
% Source page 8
% BLOG-PASSAGE: rational-dynamics-on-the-riemann-sphere:p0008
\begin{proof}
If $|\lambda|=|f'(\widehat p)|>1$, then $f$ has a local inverse fixing $\widehat p$ whose multiplier has modulus $|f'(\widehat p)|^{-1}<1$. Thus, for the corresponding open set in $S$, the orbits of its elements eventually escape.
...
\end{proof}
% Source page 9
% BLOG-PASSAGE: rational-dynamics-on-the-riemann-sphere:p0009
...
% BLOG-PASSAGE: rational-dynamics-on-the-riemann-sphere:p0010
...
% BLOG-PASSAGE: rational-dynamics-on-the-riemann-sphere:p0011
...
% BLOG-PASSAGE: rational-dynamics-on-the-riemann-sphere:p0012
...
\end{definition}
% Source page 10
% BLOG-PASSAGE: rational-dynamics-on-the-riemann-sphere:p0013
...
% BLOG-PASSAGE: rational-dynamics-on-the-riemann-sphere:p0014
...
% Source page 11
% BLOG-PASSAGE: rational-dynamics-on-the-riemann-sphere:p0015
...
% BLOG-PASSAGE: rational-dynamics-on-the-riemann-sphere:p0016
\begin{proof}
...
\end{proof}
% Source page 13
% BLOG-PASSAGE: rational-dynamics-on-the-riemann-sphere:p0017
...
\end{proof}
% Source page 14
% BLOG-PASSAGE: rational-dynamics-on-the-riemann-sphere:p0018
...
\end{proposition}
\begin{proof}
The Julia set expands open sets around it. If $z\in J(f)$ and $N$ is a neighbourhood of $z$, there is $k_0$ with $f^{\circ k_0}(N)\cap\mathcal A\ne\varnothing$. By invariance, $N\cap\mathcal A\ne\varnothing$, so $J(f)\subseteq\overline{\mathcal A}$. Since $J(f)\cap\mathcal A=\varnothing$, $J(f)\subseteq\partial\mathcal A$.

Now let $z\in\partial\mathcal A$. Every neighbourhood $N$ of $z$ meets both $\mathcal A$ and $\mathcal A^c$. For $z'\in N\cap\mathcal A$, $f^{\circ n}(z')$ converges to a point, whereas this is not so for $z''\in N\cap\mathcal A^c$. This contrast is made precise by observing that $\mathcal A$ is a union of Fatou components: if one point of a Fatou component belongs to $\mathcal A$, normality and the identity theorem show that every point of that component converges to the same attracting cycle. No point of the open set $F(f)$ can therefore lie on $\partial\mathcal A$. Hence $\partial\mathcal A\subseteq J(f)$, and $\partial\mathcal A=J(f)$.
\end{proof}
\begin{remark}
...
\end{remark}
% Source page 15
% BLOG-PASSAGE: rational-dynamics-on-the-riemann-sphere:p0019
...
\item For $z_0\in J(f)$,
\[
J(f)=\overline{\bigcup_{n\ge0}f^{-n}(z_0)}.
\]
Let $N$ be a neighbourhood of a point $z\in J(f)$. By Proposition~\ref{cd1:expansion}, some forward image of $N$ contains $z_0$, since $z_0\notin\mathcal E(f)$. Thus $f^{-n}(z_0)\cap N\ne\varnothing$ for some $n$, giving
$\overline{\bigcup_n f^{-n}(z_0)}\supseteq J(f)$. The reverse inclusion follows from invariance.
\item $J(f)$ has no isolated points. If $J(f)$ were finite, then every point of $J(f)$ would have a finite backwards orbit and hence belong to $\mathcal E(f)\subset F(f)$, which is impossible. Thus $J(f)$ is infinite and, being compact, has a limit point $z_0$. The set $\bigcup_{n\ge0}f^{-n}(z_0)$ is dense in $J(f)$. If $w\in J(f)$ were isolated, this density would give $f^{\circ n}(w)=z_0$ for some $n$. Choose distinct points $z_k\in J(f)$ with $z_k\to z_0$. Since $f^{\circ n}$ is open, every neighbourhood $U$ of $w$ has $f^{\circ n}(U)$ as a neighbourhood of $z_0$; hence, for all large $k$, there is $w_k\in U\cap J(f)$ with $f^{\circ n}(w_k)=z_k$. This contradicts the isolation of $w$.
\end{itemize}
% Source page 16
% BLOG-PASSAGE: rational-dynamics-on-the-riemann-sphere:p0020
\begin{remark}
Since $\bigcup_n f^{-n}(z_0)$ is dense in $J(f)$, for a neighbourhood $N$ of $z_0$ the set $V=\bigcup_n f^{-n}(N)$ is dense and open. For each $j\ge1$, compactness of $J(f)$ gives a finite open cover $\{N_{ij}\}_{i=1}^{M_j}$ by sets of diameter less than $1/j$. Set $V_{ij}=\bigcup_n f^{-n}(N_{ij})$; these sets are dense and open. By the Baire category theorem,
\[
\left(\bigcap_{j\ge1}\bigcap_{i=1}^{M_j}V_{ij}\right)\cap J(f)
\]
is dense in $J(f)$. If $z$ belongs to this set, then $\{z,f(z),f^2(z),\ldots\}$ meets every $N_{ij}$. The Lebesgue number lemma therefore shows that this orbit is dense in $J(f)$.
\end{remark}
...
% Source page 17
By density of the preimages of any $z_0\in J(f)$, the set of $z$ such that $(J,z)\cong(J,z_0)$ is dense in $J(f)$, unless every backwards orbit $z_0\leftarrow z_1\leftarrow z_2\leftarrow\cdots$ contains a critical point $z_k$ of $f$. Otherwise $f$ is locally univalent and one can keep backtracking $z_0$.
\end{remark}
% BLOG-CONTENT-END
