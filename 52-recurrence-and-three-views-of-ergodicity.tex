...
% BLOG-CONTENT-BEGIN
% BLOG-PASSAGE: recurrence-and-three-views-of-ergodicity:p0001
...
% BLOG-PASSAGE: recurrence-and-three-views-of-ergodicity:p0002
...
\end{theorem}
\begin{proof}
Let us first look at the points of $E$ that never return:
\[
...
% BLOG-PASSAGE: recurrence-and-three-views-of-ergodicity:p0003
\begin{remark}
...
% BLOG-PASSAGE: recurrence-and-three-views-of-ergodicity:p0004
% Source page 3
\begin{remark}
We crucially used the fact that $\mu$ is a finite measure. This turns out to
...
% BLOG-PASSAGE: recurrence-and-three-views-of-ergodicity:p0005
\begin{remark}
Define $T\colon\mathbb R^2\to\mathbb R^2$ by $T(x,y):=(x+1,y)$. Then $T$ is
$m$-invariant, where $m$ is Lebesgue measure on $\mathbb R^2$. But notice that
the sets $T^n(\mathbb D)$ escape to infinity. In such cases we have nothing in
$\mathbb R^2$ to compare $\mathbb D$ with: $\mathbb D$ runs off to infinity.
Here $m(\mathbb R^2)=\infty$, so this is not a probability
measure-preserving system in our sense. The example shows exactly where the
finiteness of the measure enters the recurrence theorem.
\end{remark}

% BLOG-PASSAGE: recurrence-and-three-views-of-ergodicity:p0006
\begin{remark}
Finite measure need not mean that the set $X$ is bounded, as shall be
illustrated now.
\end{remark}

% BLOG-PASSAGE: recurrence-and-three-views-of-ergodicity:p0007
...
% BLOG-PASSAGE: recurrence-and-three-views-of-ergodicity:p0008
% Source page 4
\begin{proof}
Let $f=\chi_B$. Then
\[
...
Now let $f\in L^\infty(\mu)$. There exist simple functions $f_n$ such that
$f_n\to f$ pointwise and $|f_n|\leq\|f\|_\infty$. The dominated convergence theorem gives
\[
  \int f\,d\mu=\int f\circ T\,d\mu
  \qquad\text{for every }f\in L^\infty(\mu).
\]
...
% BLOG-PASSAGE: recurrence-and-three-views-of-ergodicity:p0009
...
\]
is $\mu$-invariant. Indeed, the change-of-variables formula over the two inverse branches gives
\[
  \int_{-\infty}^{\infty}f(T(x))\frac{dx}{\pi(1+x^2)}
  =\int_{-\infty}^{\infty}f(y)
  \sum_{x\in T^{-1}(y)}\frac{1}{\pi(1+x^2)|T'(x)|}\,dy
  =\int_{-\infty}^{\infty}f(y)\frac{dy}{\pi(1+y^2)}.
\]
...
\]
is an isomorphism outside null sets: $\phi_*\mu=m_{S^1}$ and $\phi\circ T=T_2\circ\phi$ almost everywhere.
...
% BLOG-PASSAGE: recurrence-and-three-views-of-ergodicity:p0010
% Source page 5
...
% BLOG-PASSAGE: recurrence-and-three-views-of-ergodicity:p0011
...
% BLOG-PASSAGE: recurrence-and-three-views-of-ergodicity:p0012
...
% BLOG-PASSAGE: recurrence-and-three-views-of-ergodicity:p0013
We shall now try to understand ergodicity from three essentially distinct
perspectives, each revealing an interconnection.

% BLOG-PASSAGE: recurrence-and-three-views-of-ergodicity:p0014
\Needspace{14\baselineskip}
\subsubsection{Measure-Theoretic Ergodicity}

% BLOG-PASSAGE: recurrence-and-three-views-of-ergodicity:p0015
% Source page 6
...
\]
Therefore, almost every point $x\in X$ recurs on a given set of positive measure via
an ergodic map.
...
\[
  0<\mu(B)=\mu\left(B\cap\bigcup_{n=1}^{\infty}T^{-n}(A)\right)
...
% BLOG-PASSAGE: recurrence-and-three-views-of-ergodicity:p0016
...
$\langle f_1,U_T^*U_Tf_2\rangle=\langle f_1,f_2\rangle$, so
\[
...
A standard argument
% Source page 9
% BLOG-PASSAGE: recurrence-and-three-views-of-ergodicity:p0017
would tell you that
\[
  ...
  =\|U_T^*U_T-\id\|=0.
\]
Thus $U_T$ is an isometry, which goes by the name \emph{Koopman operator}. If
$T$ is invertible modulo null sets, then $U_T$ is unitary.

% BLOG-PASSAGE: recurrence-and-three-views-of-ergodicity:p0018
...
If $U_Tf=f$ implies that $f$ is constant almost everywhere, then $T$ is ergodic.
...
% BLOG-PASSAGE: recurrence-and-three-views-of-ergodicity:p0019
\begin{remark}
...
% BLOG-PASSAGE: recurrence-and-three-views-of-ergodicity:p0020
% Source page 10
...
% BLOG-PASSAGE: recurrence-and-three-views-of-ergodicity:p0021
...
Now, as $\sum_{\chi\in\widehat X}|c_\chi|^2$ converges to $\|f\|^2$, a
nonzero $c_\chi$ forces the orbit $\{\chi\circ T^n:n\geq0\}$ to be finite.
Since the dual map is injective, this makes $\chi$ periodic. The hypothesis
then implies $\chi\equiv1$. Hence $f$ is a constant, so $T$ is ergodic.
\end{proof}

% BLOG-PASSAGE: recurrence-and-three-views-of-ergodicity:p0022
\begin{remark}
...
with positive measure, one has the natural $T^{-1}$-invariant saturation
\[
  \bigcup_{n\geq0}T^{-n}(E)
\]
and $X$ being ergodic assures us that it covers $X$ almost everywhere. But if
$X$ were not ergodic, there is no apparent reason why an ergodic subsystem
should already be visible. If we hope to use some form of Zorn's lemma, the
existence of an ergodic subsystem is required. Thus this problem of
decomposing measure-preserving systems into ergodic components is natural. We
shall deal with it later.
\end{remark}

% BLOG-PASSAGE: recurrence-and-three-views-of-ergodicity:p0023
\begin{remark}
Using Proposition~\ref{prop:ergodicity-fourier}---I mean the ideas in it---one
can prove that the circle-doubling map and every irrational rotation map are
ergodic. Finally, as the Bernoulli system is isomorphic to the circle-doubling
system, the Bernoulli shift map is ergodic too. Thus almost all the examples
we saw are ergodic transformations.
\end{remark}
