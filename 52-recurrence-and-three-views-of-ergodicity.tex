...
The condition $\mu(X)<\infty$ may initially seem unmotivated; recurrence explains why it matters.
...
\end{theorem}
\begin{proof}
First consider the points of $E$ that never return:
\[
...
\begin{remark}
A finite measure does not require the set $X$ itself to be bounded, as the
...
$f_n\to f$ pointwise and $|f_n|\leq\|f\|_\infty$. The dominated convergence
theorem gives
\[
  \int f\,d\mu=\int f\circ T\,d\mu
  \qquad\text{for every }f\in L^\infty(\mu).
...
\]
is $\mu$-invariant. Indeed, it is easy to check that
\[
...
\]
is an isomorphism outside null sets: $\phi_*\mu=m_{S^1}$ and
$\phi\circ T=T_2\circ\phi$ almost everywhere. Hence $T$ is $\mu$-invariant.
...
infinitely often. We now ask a stronger question. Say $x\in X$ and
...
$T^{-1}(E)=E$, the orbit of $x$ cannot intersect $E$. So let us replace
...
Ergodicity admits three complementary perspectives.
...
Set $C_N=\bigcup_{n=N}^{\infty}T^{-n}(B)$. Then
\[
...
This criterion also appears in the study of mixing.
...
\[
  ...
  =\|U_T^*U_T-\id\|=0.
\]
Thus $U_T$ is an isometry, called the \emph{Koopman operator}. If $T$ is
invertible modulo null sets, then $U_T$ is unitary.
...
\begin{remark}
Thus ergodicity also has an operator-theoretic formulation. This formulation connects ergodicity with spectral properties of the Koopman operator.
\end{remark}
...
Assume the standard character expansion: for
