...
% BLOG-CONTENT-BEGIN
        ...
        Let $f$ be a rational map on the Riemann Sphere $\hat{\mathbb{C}}$ such that $deg(f) \geq 2$. In this series we shall explore three classical surgeries in complex dynamics. The first one leads up to the classification of the connected components of the Fatou set of $f$. The second one establishes the fact that the multiplier map on the hyperbolic components of a Multibrot set is a conformal isomorphism; this in its essence describes the topological shape of such sets modulo the hyperbolicity conjecture. The last one leads to the proof of the finiteness of the modular group associated to connected, forward invariant components of the Julia set of $f$.
...
\chapter{Introduction}
        ...
        In the early $20$th century Henri Poincaré and Paul Koebe proved the uniformization theorem. It essentially characterizes Riemann surfaces by the quotients of three model surfaces, they are - the unit disk, the complex plane, and the Riemann sphere. More precisely, it says the following:-
        ...
        Later in that era a French mathematician named Paul Montel proved theorems about the normality of a given family of holomorphic functions between hyperbolic surfaces. This fell in the hands of two French mathematicians - Pierre Fatou and Gaston Julia who saw that normality of the family of forward iterates of self maps $f$, namely the family $\{f^{\circ n}\}_n$, was the right tool to study stability and chaos of the deformation of the domain of $f$ (aka dynamics of $f$). Montel's Theorem along with the Uniformization Theorem reduced the number of Riemann surfaces to consider in the study of deformation: upto equivalence, they are - the Riemann sphere, the complex plane, the punctured plane, the torus, and the rest we shall refer to as hyperbolic surfaces. The dynamics on the hyperbolic surfaces have been completely classified, and handling the torus is elementary. This leaves us with only three surfaces to consider, of them, polynomial dynamics on the complex plane and the dynamics of holomorphic self maps on the Riemann sphere (i.e. rational maps) have historically seen a lot of interesting progress. Hence, we shall focus on these two\footnote{ Handling functions that have essential singularity at $\infty$ on the complex plane or the punctured plane is extremely difficult.}.     
        ...
        \begin{figure}[!htb]
            ...
            \endminipage
        \caption{Julia sets of polynomials of the form $z^2 + c$ for three particular values of $c$.}
        \end{figure}
        ...
        Their work though helped understand the stability of the dynamics of rational maps to a large extent still left a lot of open problems unattended for decades due to lack of proper tools. One of them being the following:- 
    
        \subsection*{Conjecture — No Wandering Domain} The orbits of all Fatou components of rational maps of degree atleast $2$ are eventually periodic.
        ...
        Monumental to any theory is the robustness of its classification theorems. In the case of dynamics, to be able to classify, one must first be able to compare or alter the dynamics of $f$ to grasp the parameters that control such things. One cannot alter the dynamics of $f$ within setup of conformality because of it's innate rigidity. Therefore, generalizing holomorphic maps was a natural next step (in retrospect). But historically speaking, the loosened notion of conformality (i.e. quasiconformality) entered complex dynamics quite abruptly. 

        Quasiconformal maps (abbreviated as QC-maps) are conformal (i.e angle preserving) up to a bounded error. Hence, QC-maps are much more flexible than holomorphic maps for they don't have to obey the Identity Theorem. In the $1950$s Ahlfors and Bers proved the celebrated Measurable Riemann Mapping Theorem which is essentially the Riemann Mapping Theorem for QC-maps. This theorem yields a way to associate a QC-map to a given holomorphic map. Following such a ductile association one performs a series of alterations in both the domain of the function and the function itself via a mechanism spelled out explicitly by the Surgery Principles. And finally, the crucial step in all such modes of operations is to land back in the category of holomorphic maps, which is done via Weyl's Lemma. All the aforementioned theoretical means to alter the dynamics of $f$ will be developed in \href{https://codezen1729.github.io/math-blog/\#/post/surgical-tools}{Scalpel, Forceps, and other Surgical Tools}.

        \href{https://codezen1729.github.io/math-blog/\#/post/first-surgery}{Sullivan's No Wandering Theorem} illustrates the first occurrence of a surgical procedure in complex dynamics. An American mathematician by the name Dennis Sullivan\footnote{He was awarded the Abel Prize for his groundbreaking contributions to topology in its broadest sense, and in particular its algebraic, geometric and dynamical aspects.}, inspired by Poincar\'e's perturbations of Fuchsian groups into quasi-Fuchsian groups, deployed the theory of QC-maps and resolved the No Wandering Domain conjecture entirely\footnote{With this, complex dynamics was reborn and the foundations for what is now known as Quasiconformal Surgery was laid out.}. Sullivan's technique is referred to as soft surgery. The two surgeries that follow Sullivan's are theoretically different from his original technique. In the little literature that there is about them\footnote{There is rich literature spanning both dynamics and quasiconformal maps independently; but instances of their interplay have only been of anecdotal interest in them, and a complete and comprehensive treatment of the interplay were only found in original papers.}, they are broadly referred to as cut and paste surgery. These two surgery techniques alter the dynamics of $f$ in fundamentally different ways. Cut and Paste Surgery alters the parameters of the dynamics of $f$, like - multipliers, number of periodic cycles, rotation numbers, and so on, by gluing together several maps with prescribed parameters after cutting off certain sets from their domains. Whereas, Soft Surgery merely alters the domain via a quasiconformal change of coordinates, and as we shall see in the \href{https://codezen1729.github.io/math-blog/\#/post/surgical-tools}{discussion of quasiconformal maps}, the definition of a quasiconformal map is formulated in such a way that the flexibility in its conformality is intertwined with the flexibility in changing the dynamical quantifiers. Hence, a mere change of coordinates can also alter the dynamics of $f$. 
        ...
        \href{https://codezen1729.github.io/math-blog/\#/post/second-surgery}{The Multiplier Map} aims to prove the following theorem:-
        \subsection*{Theorem — Multiplier Map} Let $\Lambda : \Omega \rightarrow \mathbb{D}$ be the multiplier map, where $\Omega$ is a hyperbolic component of period $p$ of the Mandlebrot set. Then $\Lambda$ is a conformal isomorphism.
        
        
        The objects in the theorem will be introduced below. What the theorem roughly says is that certain components of the Mandlebrot set look like bulbs. If one has the following conjecture:-
        \subsection*{Conjecture — Hyperbolicity} The interior of the Mandlebrot set is the union of its hyperbolic components.
        
        
        One can make conclusive statements about the topological shape of the Mandlebrot set, and as shall be shown, also its higher dimensional analogues - the Multibrot sets.


        \begin{figure}[htp]
            \centering
            \includegraphics[width=9cm]{M.png}
            \caption{Mandlebrot set with hyperbolic components shown as blue bulbs spread across the cardoid.}
        \end{figure}
...
\chapter{Preliminaries}
        ...
        Note that, given a Riemann surface $S$, the moment one has $S \cong \hat{S}/G$, one can forget about $S$ because the identification of the complex structures of $S$ and $\hat{S}$ via quotients is done in a well-defined manner. Therefore, to classify Riemann surfaces one must classify simply-connected Riemann surfaces and the discrete subgroups of its respective automorphism groups\footnote{This object naturally takes up the structure of a Lie group. Hence, studying discrete subgroups of certain Lie groups can yield lot of information on such classification theorems.}. By a theorem of Poincar\'e and Koebe \cite{varolin2011riemann} we know that there are only three simply connected Riemann surfaces (upto conformal equivalence), stated explicitly:-
        ...
        Riemann surfaces that admit $\mathbb{C},\hat{\mathbb{C}}$, and $\mathbb{D}$ as its universal cover are called Euclidean, Spherical, and Hyperbolic (respectively). If $G < $ Aut$(\hat{\mathbb{C})}$ acts properly discontinuously then by compactness $G = {0}$. Therefore, there is only one Spherical surface, and that is the Riemann sphere. Similarly, one can handle the case of Euclidean surfaces by noting that $G < $ Aut$(\mathbb{C}) (\cong \mathbb{C}^*\times\mathbb{C})$ is abelian and torsion free, and as the action of $G$ on $\mathbb{C}$ is a covering space action, it can't have rank $\geq 3$, for the orbits of points in $\mathbb{C}$ will then be dense. So, $G$ is isomorphic to ${0}$, $\mathbb{Z}$, or $\mathbb{Z}\times\mathbb{Z}$. Finally, upto equivalence, we have the following surfaces to study - the Riemann sphere, the complex plane, torus, cylinder (or the punctured plane), and hyperbolic surfaces. Of these, hyperbolic surfaces offer significantly more mathematical information to dynamics than the others in the lot.
        ...
        \subsection*{Theorem 2.3}{(Poincar\'e Metric on $\mathbb{H}$)}:- There exists a unique (upto multiplication by a constant) Riemannian metric on $\mathbb{H}$ which is invariant under all elements of Aut$(\mathbb{H})$.
        ...
        Finally, we can disclose the answer to the second question. \textbf{Hyperbolic surfaces endowed with this specific negatively curved geometry obey a marvellous non-increasing under holomorphic maps property.}
        ...
        Post Pick's Theorem the reason hyperbolic spaces are relevant to dynamics become evident\footnote{One stunning aspect of complex dynamics is how one classification theorem can be "aged" so to speak, and be exploited to yield others. A showing example would be the Uniformization Theorem (which classifies simply-connected Riemann surfaces), can be aged to prove Pick's Theorem (which classifies the nature of distances of holomorphic maps between hyperbolic surfaces). If one ages this further one can classify dynamics on hyperbolic surfaces. We shall see how this can be used to classify periodic Fatou components.}. Let us embark on that journey but also parallelly get some mathematical understanding of the words - order, chaos, and dynamics.
        ...
        \subsection*{Lemma 2.9}{(Ball Convergence)}:- Suppose $S \subset S'$ are Riemann surfaces with $S$ hyperbolic. Given a sequence of points $\{p_n\}_n$ which converges (in $S'$) to a boundary point $\hat{p} \in \partial S$. For any fixed $r$, one has that the entire neighbourhood of balls of radius $r$ around points in $\{p_n\}_n$ converge uniformly to $\hat{p}$.
        ...
        \end{tikzpicture}
        \caption{Neighbourhoods of points $p_n$ of a fixed radius $r$, shown in gray, converge uniformly to the boundary point $\hat{p}$.}
        \end{figure}
        
        
        These two lemmas shall be exploited vehemently for they say something phenomenal about the dynamics of hyperbolic surfaces. They say that self maps on hyperbolic surfaces show no chaotic behaviour whatsoever, even along the boundary! This knowledge about stability on hyperbolic domains will be used to study stability of deformations in general. Thus, \textbf{hyperbolic surfaces are not only objects of study but also tools in themselves.}
        ...
        on a disk that extends upto the nearest pole to $z_0$. Furthermore, if $f$ is rational then its zeroes and poles determine it entirely. Yet the philosophy says that stating almost precisely what $f$ is  is not enough to study its dynamics!
       ...
       $$ \begin{tikzcd}[column sep = large, row sep = large]
        ...
        \begin{figure}[htp]
            ...
            \caption{Julia set of $z^2 - 0.74543 + i(0.11301)$. Any pattern over here can be traced back along a dense set, except the one at critical point 0.}
        \end{figure}
        
        
        Since the Julia set is non-empty and has no isolated points, the Fatou components are hyperbolic. One can now study periodic Fatou components by replacing $f$ with a suitable iterate and procuring information from dynamics on hyperbolic surfaces. Classifying dynamics on hyperbolic surfaces in itself is not interesting because of the sheer lack of chaotic behaviour; we still do, for we have other ambitious purposes.    
        ...
        Three remarks are in order. First, one can lift $f$ to a map $F$ between its universal covers and apply Schwarz's lemma to get that all orbits of $F$ converge uniformly on compacts to the zero function which then can be passed on to $f$. Second, if $f$ is a rational map, deg$(f) \geq 2$, with a Fatou component $U$, such that for some $z \in U$, $f(z) = z$, and $|f'(z)| = 1$. Then $U \cong \mathbb{D}$, and $f$ is equivalent to an irrational rotation on $\mathbb{D}$. Reason being the following: the attracting case is off as $|f'(z)| = 1$, the escape case is off because of compactness, the finite order case is off since the degree of $f$ is $\geq 2$, and finally as $f$ admits a fixed point in $U$, it can't be equivalent to an irrational rotation on a punctured disk or an annulus. 
        ...
        \subsection*{Lemma 2.30}{(Convergence to a Boundary Fixed Point)}:- Suppose $U$ is a hyperbolic domain of a compact Riemann surface and the self map $f$ on $U$ extends continuously to $\partial U$, with atmost finite number of fixed points in $\partial U$. If some orbit of $f$ has no accumulation in $U$ then all orbits in $U$ converge to a boundary fixed point of $f$. Further, the convergence is uniform on compacts.
        ...
        \subsection*{Theorem 2.31}{(Koenig's Coordinates)}:- Let $U$ be a sufficiently small neighbourhood around a fixed point $\hat{p}$ such that $|\lambda| \notin \{0, 1\}$, then there exists  a (unique upto multiplication by a non-zero constant) conformal embedding $\phi: U \rightarrow \mathbb{C}$ such that $(z \rightarrow \lambda z) \circ \phi = \phi \circ f$.  
        
        
        \subsection*{Theorem 2.32}{(B\"ottcher Coordinates)}:- Let $U$ be a sufficiently small neighbourhood around a fixed point $\hat{p}$ such that $\lambda = 0$, then there exists  a (unique upto multiplication by a $(n-1)$st root of unity) conformal embedding $\phi: U \rightarrow \mathbb{C}$ such that $(z \rightarrow z^n) \circ \phi = \phi \circ f$, if the order of zero of the coordinate representation of $f$ at $0$ is $n$. 
       ...
       \begin{enumerate}
        ...
        \begin{figure}[htp]
            ...
            \caption{Fatou's Parabolic flower traced along a fractal-like Julia set with near $30$ petals.}
        \end{figure}
        ...
        \subsection*{Theorem 2.38}{(Fatou Coordinates)}:- For any petal $P$ $\exists$ (a unique upto translation) conformal embedding $\alpha: P \rightarrow \mathbb{C}$ such that $(z \rightarrow z+1) \circ \alpha = \alpha \circ f$. 
        
        
        
        We have seen the role played by fixed points so far, now let us explain the phenomenon of critical points, but for that, we have to restrict ourselves to case of rational maps. Critical points govern the dynamics of rational maps in three significant ways:-
        ...
        \subsection*{Definition 2.43}{(Filled Julia Set)}:- A filled Julia set is $\{z\in \mathbb{C}|$ orbit of $z$ under $f$ is bounded$\}$ (denoted usually by $K$, or $K(f)$. 
        ...
        \subsection*{Definition 2.45}{(Mandlebrot Set)}:- The Mandlebrot set $\mathcal{M}$ is defined to be the set of all $\lambda$ such that the orbit $\{f_{\lambda}^{\circ n}(0)\}_n$ is bounded.
        ...
        \begin{figure}[htp]
            ...
            \caption{Filled Julia sets determined by parameters belonging to certain hyperbolic components of the Mandlebrot set.}
        \end{figure}
        ...
        \begin{enumerate}
            ...
            \item $f^{\circ k}(f^{\circ n}(U) = f^{\circ n}(U)$ (pre-periodic Fatou component).
