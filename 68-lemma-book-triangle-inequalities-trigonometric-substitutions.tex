...
\end{lemma}
This elementary idea resolves a deceptively difficult IMOTC 2014 problem.
...
I do not encourage bombarding students with formulas. The following should be analyzed, not memorized.
...
\setcounter{theorem}{65}\begin{lemma}[Substitutions to kill problems]
...
Divide by $xyz$ and convince yourself of its validity!
\setcounter{enumi}{2}\item If $x^2+y^2+z^2-xyz=4$, use $x=a+1/a$, $y=b+1/b$, $z=c+1/c$.
Expand $xyz=(a+1/a)(b+1/b)(c+1/c)$ and verify it yourself.
...
Recall the application of Lemma 30 in the manuscript: it can be resolved without Schur's inequality.
...
% Source page 58 - left
\subsection{Pull the trigger}
There are some trigonometric identities you should be aware of. Take them as standard lemmas.
...
\paragraph{Let's bash that system!}
...
Let's pull the trigger! The sine rule for $\triangle ABP$ and $\triangle BPC$ yields the following
...
\emph{P.S.} A gifted ninth grader by the name of Sergey Yusin proposed
\unclear{the continuation is cut off at the bottom of the scan}.

% Source page 61
\paragraph{Problem 2.}
Let the incircle of $\triangle ABC$ touch $BC,CA,AB$ at $D,E,F$, respectively.
If $DI$ extended meets $EF$ at $K$, where $I$ is the incentre of $\triangle ABC$,
prove that $A,K,M$ are collinear, where $M$ is the midpoint of $BC$.

It suffices to show that $AK$ extended meets $BC$ at $M$, where $BM=MC$.
Let's proceed! Let's pull the trigger!
\begin{figure}[htbp]
...
Let $a,b,c$ be sides of a triangle similar to $ABC$. Prove
$2c^3+ab+bc+ca>a^3+b^3+c^3+3abc$.
One may prove $(a+b+c)(ab+bc+ca)+c^3>a^3+b^3+3abc$ by using
\[
a^3+b^3-c^3+3abc-\left(\sum ab\right)\left(\sum a\right)
=\left(\sum a\right)\left(\sum a^2-2\sum ab\right)<0.
\]
Now, because it is homogeneous, assume $a+b+c=1$.
This also exploits the fact that $a,b,c$ are the sides of a triangle similar to $ABC$.
The actual problem was to find the range of the area, given that $ABC$ is equilateral. Try it!
% Author check: original nonhomogeneous statement and displayed factorization are preserved.
