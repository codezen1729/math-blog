...
function sidebar(active){
  ...
  return `<aside class="journal-sidebar"><section><h2>Search</h2><button class="journal-search-button">Search the blog</button></section><section><h2>Topics</h2><ul class="journal-topics"><li><a href="blog/">All posts <span>${essays.length}</span></a></li>${series.map(item=>`<li><a href="series/${item.slug}/">${esc(item.title)}<span>${essays.filter(post=>post.phase===item.phase).length}</span></a></li>`).join('')}</ul></section>${active?`<section class="journal-series"><h2>In this series</h2><ol class="journal-contents">${entries.map(post=>`<li><a href="post/${post.slug}/">${esc(post.title)}</a></li>`).join('')}</ol></section>`:''}<section><h2>Blog Recommendations</h2><ul class="journal-topics journal-blogroll">${recommendations.map(([title,url])=>`<li><a href="${url}" target="_blank" rel="noopener noreferrer">${esc(title)}</a></li>`).join('')}</ul></section></aside>`;
}
