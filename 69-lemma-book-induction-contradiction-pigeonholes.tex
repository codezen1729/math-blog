...
Given three mutually intersecting circles, prove that their common chords are concurrent.
...
% Author check: general configurations with parallel radical axes are not discussed in source.
...
\end{lemma}
Here is a short proof. Let $|a_i-i|=|x_i|$.
We note that $x_1+x_2+\cdots+x_n=0$, which implies that the number of odd $x_i$ is even.
Therefore $\sum|x_i|$ is even. Q.E.D.!
% Author check: only |x_i| is defined in the source, before the signed-sum assertion.
...
The North Pole is one solution, but there are infinitely many others.
...
\[
\lfloor n/1\rfloor+\cdots+\lfloor n/n\rfloor+\lfloor\sqrt n\rfloor
=d(1)+d(2)+\cdots+d(n).
\]
...
% Author check: first displayed equality includes sqrt term only on LHS; subsequent identity differs and is retained separately.
% BLOG-CONTENT-END
\end{document}
