...
% BLOG-CONTENT-BEGIN
% BLOG-PASSAGE: keyhole-contour-construction:p0001
...
% BLOG-PASSAGE: keyhole-contour-construction:p0002
\begin{quote}
...
% BLOG-PASSAGE: keyhole-contour-construction:p0003
Digest this!

% BLOG-PASSAGE: keyhole-contour-construction:p0004
...
\[
  g(z_0):=\frac{1}{2\pi i}\int_\gamma\frac{f(z)}{z-z_0}\,dz
\]
is a holomorphic function of $z_0$; in fact, $g\in H(\mathbb C\setminus\gamma^*)$. It remains to show that
\[
  \left.g\right|_{U\setminus\gamma^*}=f.
\]

% BLOG-PASSAGE: keyhole-contour-construction:p0005

Then one can extend $f$ to fill in the punctures via \emph{keyhole contour integration}. There are several crucial constructions in complex analysis; of them all, the construction of a keyhole contour is foundational. Let us do that first. The idea of the extension will become clear along the way.

% BLOG-PASSAGE: keyhole-contour-construction:p0006
\subsection*{Tool 25 — Keyhole Contour Construction}

% BLOG-PASSAGE: keyhole-contour-construction:p0007
Given two simple closed polygonal paths $\gamma_1$ and $\gamma_2$ such that \(\gamma_2\subset \operatorname{int}(\gamma_1),\)
...
% BLOG-PASSAGE: keyhole-contour-construction:p0008

These $\gamma_\varepsilon$ are the ``keyholes'' which, under a pointwise limit, collapse to $\gamma_1+(-\gamma_2)$.

% BLOG-PASSAGE: keyhole-contour-construction:p0009
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.68\textwidth]{note7-fig-01.pdf}

\end{figure}

% BLOG-PASSAGE: keyhole-contour-construction:p0010
\textit{Proof.}
...
% BLOG-PASSAGE: keyhole-contour-construction:p0011
...
% BLOG-PASSAGE: keyhole-contour-construction:p0012
\begin{figure}[H]
  \centering
  \includegraphics[width=0.82\textwidth]{note7-fig-02.pdf}

\end{figure}

% BLOG-PASSAGE: keyhole-contour-construction:p0013
The keyhole is
\[
  \gamma_\varepsilon
  :=\gamma_{z_2\to a_1}
...
% BLOG-PASSAGE: keyhole-contour-construction:p0014

\textbf{Exercise.} Similarly construct a multiple keyhole.

% BLOG-PASSAGE: keyhole-contour-construction:p0015
...
% BLOG-PASSAGE: keyhole-contour-construction:p0016
\begin{figure}[H]
  \centering
  \includegraphics[width=0.54\textwidth]{note7-fig-03.pdf}

\end{figure}

% BLOG-PASSAGE: keyhole-contour-construction:p0017
...
% BLOG-PASSAGE: keyhole-contour-construction:p0018
\begin{figure}[H]
  \centering
  \includegraphics[width=0.66\textwidth]{note7-fig-04.pdf}

\end{figure}

% BLOG-PASSAGE: keyhole-contour-construction:p0019
...
% BLOG-PASSAGE: keyhole-contour-construction:p0020
Now put
\[
f_1(z):=
\begin{cases}
...
% BLOG-PASSAGE: keyhole-contour-construction:p0021

\textbf{Remark.} Although $f_1\in H(\mathbb C\setminus\gamma_1^*)$, we did not use the full power of the continuation of the integral, because we needed it to connect to $f(z)$. So we used $\operatorname{int}(\gamma_1)\cap\operatorname{ext}(\gamma_2)$ as the gluing domain where $f$ is defined and holomorphic. Notice that there is no gluing domain with which to extend $f_1$ beyond $\operatorname{int}(\gamma_1)$, because the decomposition is true only in the domain $\operatorname{int}(\gamma_1)\cap\operatorname{ext}(\gamma_2)$. Alternatively, we must consider another $\gamma_0$ in $U$ such that $\gamma_1\subset\operatorname{int}(\gamma_0)$, which is in some sense what we already have with $\gamma_1$ and $\gamma_2$.

% BLOG-PASSAGE: keyhole-contour-construction:p0022
...
% BLOG-PASSAGE: keyhole-contour-construction:p0023
...
% BLOG-PASSAGE: keyhole-contour-construction:p0024

A geometric digression is necessary to explain how this decomposition, or the keyhole contour integration method in general, can fix holes.

% BLOG-PASSAGE: keyhole-contour-construction:p0025
\subsection*{Proposition}

% BLOG-PASSAGE: keyhole-contour-construction:p0026
...
% BLOG-PASSAGE: keyhole-contour-construction:p0027
\textit{Proof.}
Let us do this via induction. For $n=2$, it is evident. Assume it holds for $n=k$. Consider $n=k+1$. Note that $\partial D_{k+1}$ will either belong to $\operatorname{int}(\gamma_{c_j})$ or to $\operatorname{ext}(\gamma_{c_j})$. The curve $\gamma_{c_j}$ will cut $\partial D_{k+1}$ at finitely many points; call them $d_1,d_2,\ldots,d_r$. Look at two consecutive points $d_i$ and $d_{i+1}$, where the $d_i$ are arranged in increasing argument, after normalizing $D_{k+1}$ to $S^1$ without loss of generality.

% BLOG-PASSAGE: keyhole-contour-construction:p0028
...
\]
If $f\in H(U\setminus S)$ is bounded, then $f$ extends holomorphically across $S$.

% BLOG-PASSAGE: keyhole-contour-construction:p0032
\textit{Proof.}
...
% BLOG-PASSAGE: keyhole-contour-construction:p0033
Note that
\[
...
% BLOG-PASSAGE: keyhole-contour-construction:p0034
\textbf{Remark.}
We saw before that
\[
\displaystyle \frac{1}{2\pi i}\int_\gamma\frac{f(z)}{z-z_0}\,dz,
...
% BLOG-PASSAGE: keyhole-contour-construction:p0035
...
% BLOG-PASSAGE: keyhole-contour-construction:p0036
...
% BLOG-PASSAGE: keyhole-contour-construction:p0037
...
% BLOG-PASSAGE: keyhole-contour-construction:p0038
...
% BLOG-PASSAGE: keyhole-contour-construction:p0039
...
% BLOG-PASSAGE: keyhole-contour-construction:p0040
...
% BLOG-PASSAGE: keyhole-contour-construction:p0041
\subsection*{Classification of Singularities}

% BLOG-PASSAGE: keyhole-contour-construction:p0042
At present we focus only on isolated singularities, that is, punctures in the domain. Let us look at an annulus of the form \(\{z:0<|z-z_0|<r\}\subset U,\)
and let $f\in H(U)$. Consider the Laurent series of $f$:
\[
  f(z)=\sum_{n=-\infty}^{\infty}a_n(z-z_0)^n.
\]

% BLOG-PASSAGE: keyhole-contour-construction:p0043
...
% BLOG-PASSAGE: keyhole-contour-construction:p0044
Now look: if $f$ has infinitely many poles tending to infinity, then $f(1/z)$ has poles clustering around $0$, so $0$ is a non-isolated singularity of $f(1/z)$. This is distinct from the third isolated class above, the essential singularity, for which the principal part has infinitely many nonzero terms.

% BLOG-PASSAGE: keyhole-contour-construction:p0045
Without loss of generality, let $0\in U\setminus S$ and let $\operatorname{int}(\gamma)$ contain all the finite poles of a meromorphic function on the Riemann sphere. If $\ell$ is a small positively oriented circle around $0$, then
\[
  \frac{1}{2\pi i}\int_\gamma f(z)\,dz
  =\sum_{z_0\in\mathbb C}\operatorname{Res}(f;z_0)
