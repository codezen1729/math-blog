...
% BLOG-CONTENT-BEGIN
Given a smooth manifold \(M\), a coordinate chart \((U,x_1,\ldots,x_n)\) assigns coordinates to points of \(U\). At each \(p\in U\), the corresponding coordinate tangent vectors \(\left.\partial/\partial x_i\right|_p\) form a basis of the tangent space \(T_pM\).

If a path allows us to associate a direction coherently at every point, for instance a smooth curve \(\gamma:[0,1]\to U\), then \(\gamma'(t)\) is a point of \(T_{\gamma(t)}M\cong\mathbb R^n\).
...
This matrix is the change-of-basis matrix between the two coordinate frames.
...
Covectors provide a dual way to measure tangent directions. Suppose the curve moves in the direction
\[
\gamma_1'(t)\left.\frac{\partial}{\partial x_1}\right|_{\gamma(t)}
+\gamma_2'(t)\left.\frac{\partial}{\partial x_2}\right|_{\gamma(t)}.
\]
The covector \(dx_2\) extracts the scalar \(\gamma_2'(t)\).
Curves \(\gamma:\mathbb R\to M\) describe motion, whereas smooth functions \(f:M\to\mathbb R\) yield covectors through their differentials.
...
=\gamma_2'(t).
\]
The \(x_2\)-component of the flow \(\gamma(t)\) is therefore given by
\((dx_2)_{\gamma(t)}(\gamma'(t))\), and \(df_p\in T_p^*M\), the dual tangent space.
Tangent vectors and covectors are organized globally by the bundles \(TM\) and \(T^*M\).
...
\]
Thus the derivative of \(f\circ\gamma\) is obtained by applying the covector \(df\) to the tangent vector \(\gamma'\). The same chain rule also controls Wirtinger derivatives.
...
\]
Now let \(\mu:\mathbb C\to\mathbb C\) be measurable with \(\|\mu\|_\infty<1\). The measurable Riemann mapping theorem gives a quasiconformal homeomorphism \(\varphi:\mathbb C\to\mathbb C\) satisfying
\[
\mu_\varphi=\frac{\partial_{\overline z}\varphi}{\partial_z\varphi}=\mu
\quad\text{almost everywhere}.
\]
Thus the ellipses generated by \(\mu\) are precisely the pullbacks under \(d\varphi\) of circles in the target tangent planes.
...
% Source page 8
This \(d_T\) is called the \emph{Teichm\"uller distance} between the marked complex planes. The chain rule therefore also tells us when a family of complex planes is close to our complex plane.
Thus the chain rule controls both the composition of Beltrami coefficients and the resulting distance between complex structures.
% BLOG-CONTENT-END
\end{document}
