...
% BLOG-PASSAGE: the-chain-rule-and-a-family-of-complex-planes:p0003
% Source page 2
...
One possible formulation uses coordinate differentials: one says that one is going in the direction \(x_i\) if
\[
dx_j(\gamma'(t))=0\quad(j\ne i).
\]
Here \(\chi_j(p)=\left.\partial/\partial x_j\right|_p\) is the global coordinate field on \(U\), and \(\gamma'(t)\) is the vector field along \(\gamma([0,1])\). The vanishing condition concerns the coordinate components; it is not, in general, the vanishing of a Lie derivative \(L_{\chi_j}(\widetilde\gamma)\).

% BLOG-PASSAGE: the-chain-rule-and-a-family-of-complex-planes:p0004
...
% BLOG-PASSAGE: the-chain-rule-and-a-family-of-complex-planes:p0005
% Source page 3
...
% BLOG-PASSAGE: the-chain-rule-and-a-family-of-complex-planes:p0006
...
\(\gamma_2'(t)\left.\partial/\partial x_2\right|_{\gamma(t)}\), which is \emph{misdirecting} its flow. The covector \(dx_2\) retrieves the scalar \(\gamma_2'(t)\), the coefficient of this extra component. To study flow, or navigation, we looked naturally at functions \(\gamma:\mathbb R\to M\). To study its dual, obstruction, focus instead on \(f:M\to\mathbb R\), \(f\in C^\infty(M)\).

% BLOG-PASSAGE: the-chain-rule-and-a-family-of-complex-planes:p0007
...
\[
df_p=\sum_{i=1}^n a_i(p)(dx_i)|_p,\qquad
...
% BLOG-PASSAGE: the-chain-rule-and-a-family-of-complex-planes:p0008
% Source page 4
...
% BLOG-PASSAGE: the-chain-rule-and-a-family-of-complex-planes:p0009
...
% BLOG-PASSAGE: the-chain-rule-and-a-family-of-complex-planes:p0010
% Source page 5
...
% BLOG-PASSAGE: the-chain-rule-and-a-family-of-complex-planes:p0011
% Source page 6
...
\[
\mu_{f\circ g}(z)=
\frac{\mu_g(z)+\mu_f(g(z))\,\dfrac{\overline{g_z(z)}}{g_z(z)}}
{1+\mu_f(g(z))\,\overline{\mu_g(z)}\,\dfrac{\overline{g_z(z)}}{g_z(z)}}.
\]
This is a consequence of the complex chain rule!

% BLOG-PASSAGE: the-chain-rule-and-a-family-of-complex-planes:p0012
% Source page 7
...
\]
\begin{figure}[H]\centering
\includegraphics[width=.64\textwidth]{cd9-fig-04.pdf}
\FigureTag{CD9.4}{fig:cd9-04}
\end{figure}
Now let \(\mu:\mathbb C\to\mathbb C\) be measurable with \(\|\mu\|_\infty<1\). A very deep theorem tells us that there exists a quasiconformal homeomorphism \(\varphi:\mathbb C\to\mathbb C\) such that
\[
(d\varphi_z)^{-1}(S^1)=\text{the ellipse given by }\mu(z)
\quad\text{almost everywhere},\qquad
\mu_\varphi=\frac{\partial_{\overline z}\varphi}{\partial_z\varphi}=\mu
\quad\text{almost everywhere}.
\]
Thus, given \(\mu\in L^\infty(\mathbb R^2)\) with \(\|\mu\|_\infty<1\), one can find a \(\varphi\) for which the ellipses generated by \(\mu\) on \(T_z\mathbb C\) are precisely those pulled back from circles in \(T_{\varphi(z)}\mathbb C\). Denote this additional structure by \((\mathbb C,\mu_\varphi)\).

% BLOG-PASSAGE: the-chain-rule-and-a-family-of-complex-planes:p0013
This generates a family of complex planes. Our familiar complex plane is
\((\mathbb C,\mu_z)\), where \(\mu_z=0\) gives circles everywhere. Define
\[
[\mu_h,\mu_f](z):=\log\bigl(K_{h\circ f^{-1}}(f(z))\bigr),
\qquad
d_{h,f}:=\sup_z[\mu_h,\mu_f](z).
\]
This is the distortion of the selected comparison map. The Teichm\"uller distance between the marked complex planes is
\[
d_T\bigl((\mathbb C,\mu_h),(\mathbb C,\mu_f)\bigr)
:=\frac12\inf_{\psi\simeq h\circ f^{-1}}\log K(\psi),
\]
where the infimum is taken over quasiconformal maps in the prescribed marking class.

% BLOG-PASSAGE: the-chain-rule-and-a-family-of-complex-planes:p0014
% Source page 8
This \(d_T\) is called the \emph{Teichm\"uller distance} between the complex planes. The chain rule therefore also tells us when a family of complex planes is close to our complex plane.
In these two ways, the chain rule quite literally chains together concepts and structures!
% BLOG-CONTENT-END
\end{document}
