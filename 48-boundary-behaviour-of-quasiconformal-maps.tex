...
Since \(h\) is \(C^1\), \(H\) is \(C^1\); \(H'\) is continuous on \(S^1\), hence uniformly continuous, so \(H\) is uniformly differentiable. In the source's abuse of notation,
\[
\lim_{x\to y}\left|\frac{H(x)-H(y)}{x-y}\right|=H'(x)=H'(y).
\]
Thus the function
\[
G(x,y)=
...
\end{definition}
We now state two theorems connecting these objects for the unit disc. Let's use this special case in other settings.
...
\end{aligned}
\]
The source uses the following bound:
\[
...
\qquad c(z)=\bar z+3i.
\]
Then \(F(x+2i)=x+\alpha_2+2i\). The picture so far is:
\begin{figure}[H]\centering
