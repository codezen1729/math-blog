...
\textbf{Remarks} (on Definition 3.1):
    ...
    \item From this point on we shall only care about real/complex finite-dimensional vector bundles. Denote by $\text{Vect}_{\mathbb{K}}^n(B)$ the category of $\mathbb{K}$-vector bundles over $B$ of dimension $n$ up to isomorphism, where $\mathbb{K}$ stands for $\mathbb{C}$ or $\mathbb{R}$.
...
\textbf{Remarks}:
    \begin{enumerate}[(i)]
        ...
        \item Let $p(E)^{\perp}$ be the fibre-wise orthogonal complement to $p(E)$, then $\big(p(E)\oplus p(E)^{\perp}, B, p(F)\oplus p(F)^{\perp}, \pi\big)$ would be isomorphic to $(B\times\mathbb{R}^N, B, \mathbb{R}^N, \pi)$. In other words, for every bundle $\eta$ over a compact Hausdorff base, there exists another bundle $\eta^{\perp}$ that cancels the twisting of $\eta$.
...
As we just saw, gauge groups play a crucial role in the construction of vector bundles as they govern how the fibres are glued together. A vector bundle acquires additional structure when its transition functions satisfy the following condition:
...
\begin{enumerate}[(i)]
    ...
    \item Let $\overline{p}^{1}(b):= \{[e,x] |  e\in p^{-1}(x)\} \cong X$, as $p^{-1}(b)\times X \rightarrow X$, the projection onto X factors through $\overline{p}^{-1}(b)$ homeomorphically. By the universal property of quotients we have a map $\psi$:
    \[\begin{tikzcd}
	{p^{-1}(b)\times X} && X \\
	& {(\overline{p})^{-1}(b)}
