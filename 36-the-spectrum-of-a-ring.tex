...
% BLOG-CONTENT-BEGIN
% BLOG-PASSAGE: the-spectrum-of-a-ring:p0001
...
\[V(I):=\{\pp\in\Spec(A)\mid I\subseteq\pp\},\qquad D(f):=\Spec(A)\setminus V((f))\quad(f\in A).\]
\end{enumerate}
\end{definition}
...
% Source page 2
% BLOG-PASSAGE: the-spectrum-of-a-ring:p0002
...
&\longleftrightarrow(x_1-\alpha_1,\ldots,x_n-\alpha_n)/I,
\quad\alpha\in\kk^n, p(\alpha)=0\text{ for every }p\in I\\
&\longleftrightarrow\text{closed points of }\Spec(A)\\
...
\begin{enumerate}[label=(\roman*)]
\setcounter{enumi}{0}\item $\Spec(\C[x])=:\mathbb A^1_{\C}$ has primes $(0)$ and $(x-a)$, $a\in\C$. Closed points correspond to all $a\in\C$. The point $(0)$ is not closed; in fact, $\overline{\{(0)\}}=\mathbb A^1_{\C}$. Such points are called \emph{generic}.
...
\setcounter{enumi}{1}\item $\Spec(\R[x])=\{(0)\}\cup\{(x-a)\mid a\in\R\}\cup\{(x^2+ax+b)\mid a,b\in\R,\ a^2-4b<0\}$. Pictorially, it looks like a folding of the complex plane.
...
\end{example}
% Source page 4
% BLOG-PASSAGE: the-spectrum-of-a-ring:p0003
Let $\varphi:A\to B$ be a ring homomorphism. It induces $f=\Spec(\varphi):\Spec(B)\to\Spec(A)$.
\begin{claim}
$f$ is continuous, so $\Spec$ is a contravariant functor.
\end{claim}
\begin{proof}
For $V(I)$ in $\Spec(A)$,
\[
...
&=\{\qq\in\Spec(B)\mid I\subseteq\varphi^{-1}(\qq)\}\\
&=\{\qq\in\Spec(B)\mid\varphi(I)\subseteq\qq\}
=V(I^e).
\end{aligned}
\]
\end{proof}
