...
\end{proof}
The proposition gives a topology on $\Spec(A)$ whose closed subsets are the $V(I)$, for ideals $I<A$. In particular, $D(f)$ is open for every $f\in A$. These are the \emph{distinguished open sets},
% Source page 2
and $\{D(f)\}_{f\in A}$ forms a basis for this topology, called the \emph{Zariski topology}.
