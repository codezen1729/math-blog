...
% BLOG-CONTENT-BEGIN
% BLOG-PASSAGE: stable-isomorphism-vector-bundles:p0001
...
% BLOG-PASSAGE: stable-isomorphism-vector-bundles:p0002
...
% BLOG-PASSAGE: stable-isomorphism-vector-bundles:p0003
...
% BLOG-PASSAGE: stable-isomorphism-vector-bundles:p0004
...
\begin{enumerate}[(i)]
    ...
    \item $\widetilde{f}([(m,n)]):= f(m)f(n)^{-1}$. $\widetilde{f}$ clearly satisfies the universal property. Indeed, the canonical map $M\rightarrow Gk(M)$ is given by $m\rightarrow [(m, e)]$, and $\widetilde{f}([(m, e)]) = f(m)$. Therefore, $\widetilde{f}([(m, n)]) = \widetilde{f}([(m, e)])\widetilde{f}([(n, e)])^{-1} = f(m)f(n)^{-1}$.
\end{enumerate}

% BLOG-PASSAGE: stable-isomorphism-vector-bundles:p0005
...
% BLOG-PASSAGE: stable-isomorphism-vector-bundles:p0006
...
In other words, the embedding theorem gives us the above elegant description of the stable isomorphism classes of vector bundles. Define
\[
  \otimes:K(X)\times K(X)\longrightarrow K(X)
\]
by
\[
[(E,\epsilon^m)]\otimes[(E',\epsilon^n)]
:=\bigl[(E\otimes E')\oplus\epsilon^{mn},
        (E\otimes\epsilon^n)\oplus(E'\otimes\epsilon^m)\bigr].
\]
From our discussion of tensor products and Whitney sums, $K$ lifts to CRings; that is, we have:
\[\begin{tikzcd}[column sep=huge,row sep=large]
	& {\text{CRings}} \\
...
% BLOG-PASSAGE: stable-isomorphism-vector-bundles:p0007
\begin{question}
Is the functor $K\in \text{PSh}(\tau_c)$ representable?
\end{question}

% BLOG-PASSAGE: stable-isomorphism-vector-bundles:p0008
\underline{\textbf{Representation of the $K$-functor}}

% BLOG-PASSAGE: stable-isomorphism-vector-bundles:p0009
...
% BLOG-PASSAGE: stable-isomorphism-vector-bundles:p0010
Then
...
% BLOG-PASSAGE: stable-isomorphism-vector-bundles:p0011
\begin{remark}[Notation]
Recall that for a group $G$, we denote by $BG$ its classifying space, and by $EG$ the universal bundle. It turns out that, for $U:= \text{colim} U(n)$, the classifying space $BU \simeq Gr(\infty, \mathbb{C}^{\infty})$, and $BU(n) \simeq Gr(n, \mathbb{C}^{\infty})$, so we shall use the symbol $BU$ for $Gr(\infty, \mathbb{C}^{\infty})$. The corresponding real classifying spaces are denoted by $BO$ and $BO(n)$. In the remainder of this post, ``bundle'' means a finite-dimensional complex vector bundle over a compact Hausdorff space.
\end{remark}

% BLOG-PASSAGE: stable-isomorphism-vector-bundles:p0012
\begin{remark}
Note that $(\dagger)$ does not encode the ring structure of $K(X)$. To say that $[X, BU]$ with the usual direct sum and tensor product of maps induced point-wise by $BU$ is isomorphic to $\widetilde{K}(X)$, we need to check the following two things:
\begin{enumerate}[(i)]
    \item $(f\oplus' g)^*\gamma_{n+m} \cong f^*\gamma_n\oplus g^*\gamma_m$
    \item $(f\otimes' g)^*\gamma_{nm} \cong f^*\gamma_n\otimes g^*\gamma_m$
\end{enumerate}
where $\oplus'$ and $\otimes'$ denote the sum and product in the ring of maps. The isomorphisms (i) and (ii) are nearly tautological if one appeals to the tautological bundle. Then, passing to colimits would give us an isomorphism that preserves the algebraic structure.
\end{remark}
% BLOG-CONTENT-END
