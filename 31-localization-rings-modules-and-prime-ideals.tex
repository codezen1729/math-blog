...
% BLOG-CONTENT-BEGIN
% BLOG-PASSAGE: localization-rings-modules-and-prime-ideals:p0001
\subsection{Motivations for localization}
Recall the construction of $\Q$ from $\Z$:
\[
...
\[
(a,s)\sim(b,t)\ \Longleftrightarrow\ (at-bs)u=0\quad\text{for some }u\in S.
\]
Set $S^{-1}A:=(A\times S)/{\sim}$, the \emph{localization of $A$ by $S$}.
\end{definition}
\begin{remark}
...
\item Every element of $S^{-1}A$ is of the form $f(a)f(s)^{-1}$.
\end{itemize}
These properties determine the initial object in (iii) uniquely up to isomorphism.
\end{enumerate}
\end{remark}
...
% Source page 3
% BLOG-PASSAGE: localization-rings-modules-and-prime-ideals:p0002
For $f:M\to N$, one has $S^{-1}f:S^{-1}M\to S^{-1}N$, $m/s\mapsto f(m)/s$.
...
\]
\setcounter{example}{5}\begin{example}[Local properties]
\begin{enumerate}[label=(\roman*)]
\setcounter{enumi}{0}\item $M=0\Longleftrightarrow M_{\pp}=0\Longleftrightarrow M_{\mm}=0$ for every maximal $\mm$. To see this, let $x\in M$, $x\ne0$, and $\aa=\Ann(x)$, say $\aa\subseteq\mm$. Then $x/1\ne0$ in $M_{\mm}$: otherwise, $x$ would be killed by some element in $A\setminus\mm$, contradicting $\Ann(x)\subseteq\mm$.
\setcounter{enumi}{1}\item A map $\varphi:M\to N$ is injective if and only if every localized map $\varphi_{\pp}:M_{\pp}\to N_{\pp}$ is injective; equivalently, it is enough to check every maximal ideal $\mm$. Indeed, $\ker\varphi=0\Longleftrightarrow\ker\varphi_{\pp}=0\Longleftrightarrow\ker\varphi_{\mm}=0$.
\setcounter{enumi}{2}\item Surjectivity of $\varphi:M\to N$ is also a local property.
\end{enumerate}
\end{example}
...
\end{aligned}
\]
(iii) $\aa\in\mathcal C\Longleftrightarrow\aa^{ec}\subseteq\aa\Longleftrightarrow(sx\in\aa\text{ for some }s\in S\Rightarrow x\in\aa)$. In other words, extend $\aa$ to $S^{-1}A$, use the invertibility of $s\in S$, and then contract the resulting ideal back to $A$. This is equivalent to no $s\in S$ being a zero-divisor in $A/\aa$.
