...
We want to generalize this to arbitrary commutative rings. If $X$ is a variety and $x\in X$, then a neighbourhood $U$ of $x$ corresponds to a localization of the coordinate ring $A(X)$. Thus localization is the geometric equivalent of seeing a point locally.
...
\end{example}
Localization can be attempted on $A$-modules in exactly the same way. For an $A$-module $M$, let
...
\setcounter{enumi}{1}\item A map $\varphi:M\to N$ is injective if and only if $\varphi_{\pp}:M_{\pp}\to N_{\pp}$ is injective, if and only if $\varphi_{\mm}:M_{\mm}\to N_{\mm}$ is injective. Indeed, $\ker\varphi=0\Longleftrightarrow\ker\varphi_{\pp}=0\Longleftrightarrow\ker\varphi_{\mm}=0$.
