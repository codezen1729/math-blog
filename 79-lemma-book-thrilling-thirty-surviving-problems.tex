...
\setcounter{enumi}{1}\item Given a triangle $ABC$ in a plane $R$, construct a geometrical entity $\Psi$
...
\setcounter{enumi}{2}\item A cylinder fits a complete sphere exactly. Another sphere of maximum radius
...
\setcounter{enumi}{3}\item Determine the number of ways to arrange circles subject to the following conditions:
\begin{itemize}
...
\setcounter{enumi}{4}\item Let $f$ be a function such that $f(1)=y_0$ and $f(n)=h(f(n-1))$, where $h$
...
\setcounter{enumi}{5}\item There are infinitely many boxes, numbered $1,2,3,\ldots$. Put balls in each
box so that the GCD of the numbers of balls in any two boxes equals the GCD of their
box numbers. Pick the box numbered with your year of birth and find the number of balls in it.
...
\setcounter{enumi}{8}\item Given two circles intersecting at $X$ and $Y$, draw a chord $AB$ in the first
...
\setcounter{enumi}{10}\item Prove the following binomial identity:
\[
\frac{\binom n0}{3}-\frac{\binom n1}{4}+\frac{\binom n2}{5}
-\frac{\binom n3}{6}+\cdots\infty
=\frac2{(n+1)(n+2)(n+3)}.
\]
...
\setcounter{enumi}{12}\item If $\Psi(x)$ is a function of $x$ defined by
...
\setcounter{enumi}{13}\item If $a,b,c$ are positive reals such that $\sqrt a+\sqrt b+\sqrt c=3\sqrt2$\ldots
\par\unclear{the remainder of Problem 14 is absent from the supplied scan}.

\item[\textbf{15--17.}] \unclear{these problems are absent from the supplied scan}.
\setcounter{enumi}{17}
% Source page 66
% Source page 67 - duplicate of page 66; no new content.
\setcounter{enumi}{17}\item \unclear{the beginning of Problem 18 is absent; only the following closing fragment survives}.
\par\emph{And he told me, ``We can write $(2xz=xz+xz)$!!'' to which I responded,
``Are you serious!''}

\setcounter{enumi}{18}\item Given positive reals $a,b,c,d$ satisfying
...
\]
\emph{An inspiration from Vedant's problem!}
...
\setcounter{enumi}{28}\item Let $(x_1,x_2,\ldots,x_k)\in\mathbb R$. Prove that the following inequality
...
\setcounter{enumi}{29}\item Define $x_k=(a^k+b^k+c^k)/3$, where $a,b,c$ are positive reals. Prove
\[
\frac{x_1^5}{x_2x_3}+\frac{x_2^2}{x_1x_3}+\frac{x_3}{x_1x_2}>3.
\]
\end{enumerate}
